-- THIS FILE IS GENERATED
local _, sc = ...;
if sc.class ~= sc.classes.druid then return; end
local powers = sc.powers
local schools = sc.schools
local spell_flags = sc.spell_flags
local comp_flags = sc.comp_flags
sc.equippable_weapons_mask = 42032;
sc.equippable_armors_mask = 7;
sc.spells = {
	[5179] = {
		direct = {
			min = 63,
			max = 73,
			school1 = schools.nature,
			coef = 0.571,
			per_lvl = 1,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 2,
		cost = 70,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 22,
		lvl_max = 28,
		lvl_outdated = 29,
		base_id = 5176,
		gcd = 1.5000001,
		train = 3000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[21849] = {
		cast_time = 0,
		cost = 900,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 50,
		lvl_max = 70,
		lvl_outdated = 59,
		base_id = 21849,
		gcd = 1.5000001,
		train = 23000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[9888] = {
		direct = {
			min = 1516,
			max = 1796,
			school1 = schools.nature,
			coef = 1,
			per_lvl = 5.9,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 600,
		power_type = powers.mana,
		rank = 9,
		lvl_req = 50,
		lvl_max = 55,
		lvl_outdated = 55,
		base_id = 5185,
		gcd = 1.5000001,
		train = 23000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[26999] = {
		periodic = {
			min = 25,
			max = 25,
			tick_time = 1,
			dur = 10,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.no_attack, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 65,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 22842,
		gcd = 1.5000001,
		train = 59000,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.eval),
	},
	[27685] = {
		cast_time = 0,
		cost = 35,
		power_type = powers.focus,
		rank = 6,
		lvl_req = 60,
		lvl_max = 66,
		lvl_outdated = 70,
		base_id = 7371,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.pet),
	},
	[9839] = {
		periodic = {
			min = 122,
			max = 122,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.2,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 235,
		power_type = powers.mana,
		rank = 8,
		lvl_req = 46,
		lvl_max = 51,
		lvl_outdated = 51,
		base_id = 774,
		gcd = 1.5000001,
		train = 20000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[22896] = {
		periodic = {
			min = 20,
			max = 20,
			tick_time = 1,
			dur = 10,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.no_attack, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 56,
		lvl_max = 70,
		lvl_outdated = 64,
		base_id = 22842,
		gcd = 1.5000001,
		train = 30000,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.eval),
	},
	[29166] = {
		periodic = {
			min = 4,
			max = 4,
			tick_time = 2,
			dur = 20,
			school1 = schools.nature,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 0.04,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 40,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 29166,
		gcd = 1.5000001,
		train = 14000,
		flags = bit.bor(0, spell_flags.cd, spell_flags.base_mana_cost, spell_flags.instant, spell_flags.resource_regen, spell_flags.regen_pct),
	},
	[9884] = {
		cast_time = 0,
		cost = 340,
		power_type = powers.mana,
		rank = 6,
		lvl_req = 50,
		lvl_max = 70,
		lvl_outdated = 59,
		base_id = 1126,
		gcd = 1.5000001,
		train = 23000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[1082] = {
		direct = {
			base_min = 27,
			base_max = 27,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 45,
		power_type = powers.energy,
		rank = 1,
		lvl_req = 20,
		lvl_max = 70,
		lvl_outdated = 27,
		base_id = 1082,
		gcd = 1,
		train = 2000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[6603] = {
		direct = {
			base_min = 0,
			base_max = 0,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.applies_oh),
		},
		cast_time = 0,
		cost = 0,
		power_type = powers.rage,
		rank = 0,
		lvl_req = 0,
		lvl_max = 0,
		lvl_outdated = 70,
		base_id = 6603,
		gcd = 0,
		train = -1,
		flags = bit.bor(0, spell_flags.eval, spell_flags.uses_attack_speed),
	},
	[9876] = {
		direct = {
			min = 445,
			max = 525,
			school1 = schools.arcane,
			coef = 1,
			per_lvl = 3,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 315,
		power_type = powers.mana,
		rank = 6,
		lvl_req = 58,
		lvl_max = 64,
		lvl_outdated = 59,
		base_id = 2912,
		gcd = 1.5000001,
		train = 32000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[8972] = {
		direct = {
			base_min = 49,
			base_max = 49,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 15,
		power_type = powers.rage,
		rank = 4,
		lvl_req = 34,
		lvl_max = 70,
		lvl_outdated = 41,
		base_id = 6807,
		gcd = 0,
		train = 10000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.on_next_attack, spell_flags.uses_attack_speed, spell_flags.refund_on_miss),
	},
	[6808] = {
		direct = {
			base_min = 27,
			base_max = 27,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 15,
		power_type = powers.rage,
		rank = 2,
		lvl_req = 18,
		lvl_max = 70,
		lvl_outdated = 25,
		base_id = 6807,
		gcd = 0,
		train = 1900,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.on_next_attack, spell_flags.uses_attack_speed, spell_flags.refund_on_miss),
	},
	[22570] = {
		direct = {
			base_min = 45,
			base_max = 45,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 84,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 35,
		power_type = powers.energy,
		rank = 1,
		lvl_req = 62,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 22570,
		gcd = 1,
		train = 43000,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.binary, spell_flags.eval, spell_flags.finishing_move_dmg, spell_flags.finishing_move_dur),
	},
	[33763] = {
		direct = {
			min = 600,
			max = 600,
			school1 = schools.nature,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		periodic = {
			min = 39,
			max = 39,
			tick_time = 1,
			dur = 7.0000005,
			school1 = schools.nature,
			coef = 0.0742,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 220,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 64,
		lvl_max = 69,
		lvl_outdated = 70,
		base_id = 33763,
		gcd = 1.5000001,
		train = 53000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[26995] = {
		cast_time = 1.5000001,
		cost = 140,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 70,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 2908,
		gcd = 1.5000001,
		train = 100000,
		anyschool = schools.nature,
		flags = bit.bor(0),
	},
	[8992] = {
		direct = {
			base_min = 44,
			base_max = 44,
			min = 2.25,
			max = 2.25,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 60,
		power_type = powers.energy,
		rank = 3,
		lvl_req = 38,
		lvl_max = 70,
		lvl_outdated = 45,
		base_id = 5221,
		gcd = 1,
		train = 12000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss, spell_flags.behind_target),
	},
	[2908] = {
		cast_time = 1.5000001,
		cost = 50,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 22,
		lvl_max = 70,
		lvl_outdated = 37,
		base_id = 2908,
		gcd = 1.5000001,
		train = 3000,
		anyschool = schools.nature,
		flags = bit.bor(0),
	},
	[9857] = {
		direct = {
			min = 809,
			max = 905,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 6,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		periodic = {
			min = 123,
			max = 123,
			tick_time = 3.0000002,
			dur = 21.000002,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 2,
		cost = 485,
		power_type = powers.mana,
		rank = 8,
		lvl_req = 54,
		lvl_max = 59,
		lvl_outdated = 59,
		base_id = 8936,
		gcd = 1.5000001,
		train = 28000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[8955] = {
		cast_time = 1.5000001,
		cost = 75,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 38,
		lvl_max = 70,
		lvl_outdated = 53,
		base_id = 2908,
		gcd = 1.5000001,
		train = 12000,
		anyschool = schools.nature,
		flags = bit.bor(0),
	},
	[16857] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 25,
		lvl_max = 70,
		lvl_outdated = 29,
		base_id = 16857,
		gcd = 1,
		train = -1,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[81] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 1,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 81,
		gcd = 0,
		train = -1,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.ehp),
	},
	[20742] = {
		cast_time = 2,
		cost = 0.68,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 40,
		lvl_max = 70,
		lvl_outdated = 49,
		base_id = 20484,
		gcd = 1.5000001,
		train = 14000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.base_mana_cost),
	},
	[9005] = {
		periodic = {
			min = 45,
			max = 45,
			tick_time = 3.0000002,
			dur = 18,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.no_attack, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 50,
		power_type = powers.energy,
		rank = 1,
		lvl_req = 36,
		lvl_max = 70,
		lvl_outdated = 45,
		base_id = 9005,
		gcd = 1,
		train = 11000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.binary, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[26179] = {
		cast_time = 0,
		cost = 35,
		power_type = powers.focus,
		rank = 4,
		lvl_req = 36,
		lvl_max = 40,
		lvl_outdated = 47,
		base_id = 7371,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.pet),
	},
	[27011] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 66,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 16857,
		gcd = 1,
		train = 1700,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[26987] = {
		direct = {
			min = 220,
			max = 258,
			school1 = schools.arcane,
			coef = 0.15,
			per_lvl = 3.6,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 111,
			max = 111,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.arcane,
			coef = 0.13,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 430,
		power_type = powers.mana,
		rank = 11,
		lvl_req = 63,
		lvl_max = 68,
		lvl_outdated = 69,
		base_id = 8921,
		gcd = 1.5000001,
		train = 48000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[17402] = {
		periodic = {
			min = 134,
			max = 134,
			tick_time = 1,
			dur = 10,
			school1 = schools.nature,
			coef = 0.129,
			per_lvl = 0.3,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit),
		},
		cast_time = 0,
		cost = 1495,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 60,
		lvl_max = 66,
		lvl_outdated = 69,
		base_id = 16914,
		gcd = 1.5000001,
		train = 34000,
		flags = bit.bor(0, spell_flags.channel, spell_flags.cd, spell_flags.eval, spell_flags.ot_haste_improved),
	},
	[9823] = {
		periodic = {
			min = 55,
			max = 55,
			tick_time = 3.0000002,
			dur = 18,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.no_attack, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 50,
		power_type = powers.energy,
		rank = 2,
		lvl_req = 46,
		lvl_max = 70,
		lvl_outdated = 55,
		base_id = 9005,
		gcd = 1,
		train = 20000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.binary, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[20554] = {
		direct = {
			min = 25,
			max = 25,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 0.06,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 0,
		lvl_max = 0,
		race_flags = 128,
		lvl_outdated = 70,
		base_id = 20554,
		gcd = 0,
		train = 0,
		flags = bit.bor(0, spell_flags.cd, spell_flags.base_mana_cost, spell_flags.instant),
	},
	[26998] = {
		direct = {
			threat_mod_flat = 0,
			school1 = schools.physical,
			flags = bit.bor(0, comp_flags.unbounded_aoe, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 10,
		power_type = powers.rage,
		rank = 6,
		lvl_req = 62,
		lvl_max = 72,
		lvl_outdated = 70,
		base_id = 99,
		gcd = 1.5000001,
		train = 43000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.instant, spell_flags.only_threat),
	},
	[16979] = {
		cast_time = 0,
		cost = 5,
		power_type = powers.rage,
		rank = 0,
		lvl_req = 20,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 16979,
		gcd = 0,
		train = 0,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[1850] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 26,
		lvl_max = 70,
		lvl_outdated = 45,
		base_id = 1850,
		gcd = 0,
		train = 4500,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant),
	},
	[6787] = {
		direct = {
			base_min = 62,
			base_max = 62,
			min = 3.85,
			max = 3.85,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.no_active_defense, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 60,
		power_type = powers.energy,
		rank = 2,
		lvl_req = 42,
		lvl_max = 70,
		lvl_outdated = 49,
		base_id = 6785,
		gcd = 1,
		train = 16000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[27012] = {
		periodic = {
			min = 206,
			max = 206,
			tick_time = 1,
			dur = 10,
			school1 = schools.nature,
			coef = 0.129,
			per_lvl = 0.4,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit),
		},
		cast_time = 0,
		cost = 1905,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 70,
		lvl_max = 76,
		lvl_outdated = 70,
		base_id = 16914,
		gcd = 1.5000001,
		train = 100000,
		flags = bit.bor(0, spell_flags.channel, spell_flags.cd, spell_flags.eval, spell_flags.ot_haste_improved),
	},
	[17391] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 42,
		lvl_max = 70,
		lvl_outdated = 53,
		base_id = 16857,
		gcd = 1,
		train = 800,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[8939] = {
		direct = {
			min = 240,
			max = 274,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 3.1,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		periodic = {
			min = 37,
			max = 37,
			tick_time = 3.0000002,
			dur = 21.000002,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 2,
		cost = 185,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 24,
		lvl_max = 29,
		lvl_outdated = 29,
		base_id = 8936,
		gcd = 1.5000001,
		train = 4000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[2782] = {
		cast_time = 0,
		cost = 0.08,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 24,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 2782,
		gcd = 1.5000001,
		train = 4000,
		anyschool = schools.arcane,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.instant),
	},
	[22827] = {
		direct = {
			min = 20,
			max = 44,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 59,
			jump_amp = 1.5,
			flags = bit.bor(0, comp_flags.jump_amp_as_per_extra_power),
		},
		cast_time = 0,
		cost = 35,
		power_type = powers.energy,
		rank = 2,
		lvl_req = 40,
		lvl_max = 70,
		lvl_outdated = 47,
		base_id = 22568,
		gcd = 1,
		train = 14000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[774] = {
		periodic = {
			min = 8,
			max = 8,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.08,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 25,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 4,
		lvl_max = 9,
		lvl_outdated = 9,
		base_id = 774,
		gcd = 1.5000001,
		train = 100,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[9840] = {
		periodic = {
			min = 152,
			max = 152,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.2,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 280,
		power_type = powers.mana,
		rank = 9,
		lvl_req = 52,
		lvl_max = 57,
		lvl_outdated = 57,
		base_id = 774,
		gcd = 1.5000001,
		train = 26000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[5234] = {
		cast_time = 0,
		cost = 160,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 30,
		lvl_max = 70,
		lvl_outdated = 39,
		base_id = 1126,
		gcd = 1.5000001,
		train = 6000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[33876] = {
		direct = {
			base_min = 99,
			base_max = 99,
			min = 1.5999999,
			max = 1.5999999,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 45,
		power_type = powers.energy,
		rank = 1,
		lvl_req = 50,
		lvl_max = 70,
		lvl_outdated = 57,
		base_id = 33876,
		gcd = 1,
		train = -1,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[9880] = {
		direct = {
			base_min = 101,
			base_max = 101,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 15,
		power_type = powers.rage,
		rank = 6,
		lvl_req = 50,
		lvl_max = 70,
		lvl_outdated = 57,
		base_id = 6807,
		gcd = 0,
		train = 23000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.on_next_attack, spell_flags.uses_attack_speed, spell_flags.refund_on_miss),
	},
	[9901] = {
		cast_time = 1.5000001,
		cost = 100,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 54,
		lvl_max = 70,
		lvl_outdated = 69,
		base_id = 2908,
		gcd = 1.5000001,
		train = 28000,
		anyschool = schools.nature,
		flags = bit.bor(0),
	},
	[9898] = {
		direct = {
			threat_mod_flat = 0,
			school1 = schools.physical,
			flags = bit.bor(0, comp_flags.unbounded_aoe, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 10,
		power_type = powers.rage,
		rank = 5,
		lvl_req = 52,
		lvl_max = 62,
		lvl_outdated = 61,
		base_id = 99,
		gcd = 1.5000001,
		train = 26000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.instant, spell_flags.only_threat),
	},
	[6783] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 40,
		lvl_max = 59,
		lvl_outdated = 59,
		base_id = 5215,
		gcd = 0,
		train = 14000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.binary),
	},
	[2090] = {
		periodic = {
			min = 45,
			max = 45,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.2,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 105,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 22,
		lvl_max = 27,
		lvl_outdated = 27,
		base_id = 774,
		gcd = 1.5000001,
		train = 3000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[8951] = {
		direct = {
			min = 280,
			max = 334,
			school1 = schools.arcane,
			coef = 1,
			per_lvl = 2.3,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 230,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 42,
		lvl_max = 48,
		lvl_outdated = 49,
		base_id = 2912,
		gcd = 1.5000001,
		train = 16000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[24976] = {
		periodic = {
			min = 72,
			max = 72,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.127,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic, comp_flags.dot_resi_pen),
		},
		cast_time = 0,
		cost = 135,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 50,
		lvl_max = 59,
		lvl_outdated = 59,
		base_id = 5570,
		gcd = 1.5000001,
		train = 1150,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.talent),
	},
	[5232] = {
		cast_time = 0,
		cost = 50,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 10,
		lvl_max = 70,
		lvl_outdated = 19,
		base_id = 1126,
		gcd = 1.5000001,
		train = 300,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[18960] = {
		cast_time = 10,
		cost = 120,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 10,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 18960,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.arcane,
		flags = bit.bor(0),
	},
	[33987] = {
		direct = {
			base_min = 135,
			base_max = 135,
			min = 1.15,
			max = 1.15,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.rage,
		rank = 3,
		lvl_req = 68,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 33878,
		gcd = 1.5000001,
		train = 1900,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[6780] = {
		direct = {
			min = 139,
			max = 157,
			school1 = schools.nature,
			coef = 0.571,
			per_lvl = 1.6,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 2,
		cost = 125,
		power_type = powers.mana,
		rank = 6,
		lvl_req = 38,
		lvl_max = 44,
		lvl_outdated = 45,
		base_id = 5176,
		gcd = 1.5000001,
		train = 12000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[20748] = {
		cast_time = 2,
		cost = 0.68,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 60,
		lvl_max = 70,
		lvl_outdated = 68,
		base_id = 20484,
		gcd = 1.5000001,
		train = 34000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.base_mana_cost),
	},
	[26201] = {
		cast_time = 0,
		cost = 35,
		power_type = powers.focus,
		rank = 5,
		lvl_req = 48,
		lvl_max = 54,
		lvl_outdated = 59,
		base_id = 7371,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.pet),
	},
	[9747] = {
		direct = {
			threat_mod_flat = 0,
			school1 = schools.physical,
			flags = bit.bor(0, comp_flags.unbounded_aoe, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 10,
		power_type = powers.rage,
		rank = 4,
		lvl_req = 42,
		lvl_max = 52,
		lvl_outdated = 51,
		base_id = 99,
		gcd = 1.5000001,
		train = 16000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.instant, spell_flags.only_threat),
	},
	[768] = {
		periodic = {
			min = -11,
			max = -11,
			tick_time = 2,
			dur = -0.001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 0.35,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 20,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 768,
		gcd = 1.5000001,
		train = 2000,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.instant, spell_flags.resource_regen),
	},
	[16813] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 48,
		lvl_max = 70,
		lvl_outdated = 57,
		base_id = 16689,
		gcd = 1.5000001,
		train = 1100,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[9750] = {
		direct = {
			min = 511,
			max = 575,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 4.7,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		periodic = {
			min = 78,
			max = 78,
			tick_time = 3.0000002,
			dur = 21.000002,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 2,
		cost = 335,
		power_type = powers.mana,
		rank = 6,
		lvl_req = 42,
		lvl_max = 47,
		lvl_outdated = 47,
		base_id = 8936,
		gcd = 1.5000001,
		train = 16000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[33745] = {
		direct = {
			min = 31,
			max = 31,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 31,
			max = 31,
			tick_time = 3.0000002,
			dur = 15.000001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 15,
		power_type = powers.rage,
		rank = 1,
		lvl_req = 66,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 33745,
		gcd = 1.5000001,
		train = 66000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[9756] = {
		cast_time = 0,
		cost = 240,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 44,
		lvl_max = 70,
		lvl_outdated = 53,
		base_id = 467,
		gcd = 1.5000001,
		train = 18000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[3627] = {
		periodic = {
			min = 76,
			max = 76,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.2,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 160,
		power_type = powers.mana,
		rank = 6,
		lvl_req = 34,
		lvl_max = 39,
		lvl_outdated = 39,
		base_id = 774,
		gcd = 1.5000001,
		train = 10000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[1058] = {
		periodic = {
			min = 14,
			max = 14,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.125,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 40,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 10,
		lvl_max = 15,
		lvl_outdated = 15,
		base_id = 774,
		gcd = 1.5000001,
		train = 300,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[26177] = {
		cast_time = 0,
		cost = 35,
		power_type = powers.focus,
		rank = 2,
		lvl_req = 12,
		lvl_max = 18,
		lvl_outdated = 23,
		base_id = 7371,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.pet),
	},
	[27000] = {
		direct = {
			base_min = 190,
			base_max = 190,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 45,
		power_type = powers.energy,
		rank = 6,
		lvl_req = 67,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 1082,
		gcd = 1,
		train = 73000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[33943] = {
		periodic = {
			min = -11,
			max = -11,
			tick_time = 2,
			dur = -0.001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 0.13,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 68,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 33943,
		gcd = 1.5000001,
		train = 81000,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.instant, spell_flags.resource_regen),
	},
	[1735] = {
		direct = {
			threat_mod_flat = 0,
			school1 = schools.physical,
			flags = bit.bor(0, comp_flags.unbounded_aoe, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 10,
		power_type = powers.rage,
		rank = 2,
		lvl_req = 20,
		lvl_max = 30,
		lvl_outdated = 31,
		base_id = 99,
		gcd = 1.5000001,
		train = 2000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.instant, spell_flags.only_threat),
	},
	[5229] = {
		direct = {
			min = -75,
			max = -75,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 12,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 5229,
		gcd = 0,
		train = 800,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant),
	},
	[769] = {
		direct = {
			min = 21,
			max = 21,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jumps = 2,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.native_jumps),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.rage,
		rank = 3,
		lvl_req = 34,
		lvl_max = 70,
		lvl_outdated = 43,
		base_id = 779,
		gcd = 1.5000001,
		train = 10000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[9492] = {
		periodic = {
			min = 4,
			max = 4,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 7,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 30,
		power_type = powers.energy,
		rank = 2,
		lvl_req = 28,
		lvl_max = 70,
		lvl_outdated = 35,
		base_id = 1079,
		gcd = 1,
		train = 5000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[3029] = {
		direct = {
			base_min = 39,
			base_max = 39,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 45,
		power_type = powers.energy,
		rank = 2,
		lvl_req = 28,
		lvl_max = 70,
		lvl_outdated = 37,
		base_id = 1082,
		gcd = 1,
		train = 5000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[33878] = {
		direct = {
			base_min = 75,
			base_max = 75,
			min = 1.15,
			max = 1.15,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.rage,
		rank = 1,
		lvl_req = 50,
		lvl_max = 70,
		lvl_outdated = 57,
		base_id = 33878,
		gcd = 1.5000001,
		train = -1,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[9827] = {
		periodic = {
			min = 75,
			max = 75,
			tick_time = 3.0000002,
			dur = 18,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.no_attack, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 50,
		power_type = powers.energy,
		rank = 3,
		lvl_req = 56,
		lvl_max = 70,
		lvl_outdated = 65,
		base_id = 9005,
		gcd = 1,
		train = 30000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.binary, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[9834] = {
		direct = {
			min = 157,
			max = 185,
			school1 = schools.arcane,
			coef = 0.15,
			per_lvl = 3,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 80,
			max = 80,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.arcane,
			coef = 0.13,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 325,
		power_type = powers.mana,
		rank = 9,
		lvl_req = 52,
		lvl_max = 57,
		lvl_outdated = 57,
		base_id = 8921,
		gcd = 1.5000001,
		train = 26000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[21850] = {
		cast_time = 0,
		cost = 1200,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 60,
		lvl_max = 70,
		lvl_outdated = 69,
		base_id = 21849,
		gcd = 1.5000001,
		train = 34000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[1062] = {
		periodic = {
			min = 10,
			max = 10,
			tick_time = 3.0000002,
			dur = 15.000001,
			school1 = schools.nature,
			coef = 0.093,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 1.5000001,
		cost = 65,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 18,
		lvl_max = 70,
		lvl_outdated = 27,
		base_id = 339,
		gcd = 1.5000001,
		train = 1900,
		flags = bit.bor(0, spell_flags.binary, spell_flags.eval),
	},
	[33891] = {
		direct = {
			min = 25,
			max = 25,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jumps = 4,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 0.28,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 50,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 33891,
		gcd = 1.5000001,
		train = 0,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.instant, spell_flags.talent),
	},
	[22895] = {
		periodic = {
			min = 15,
			max = 15,
			tick_time = 1,
			dur = 10,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.no_attack, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 46,
		lvl_max = 70,
		lvl_outdated = 55,
		base_id = 22842,
		gcd = 1.5000001,
		train = 20000,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.eval),
	},
	[9881] = {
		direct = {
			base_min = 128,
			base_max = 128,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 15,
		power_type = powers.rage,
		rank = 7,
		lvl_req = 58,
		lvl_max = 70,
		lvl_outdated = 66,
		base_id = 6807,
		gcd = 0,
		train = 32000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.on_next_attack, spell_flags.uses_attack_speed, spell_flags.refund_on_miss),
	},
	[5178] = {
		direct = {
			min = 44,
			max = 52,
			school1 = schools.nature,
			coef = 0.443,
			per_lvl = 0.8,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 2,
		cost = 55,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 14,
		lvl_max = 20,
		lvl_outdated = 21,
		base_id = 5176,
		gcd = 1.5000001,
		train = 900,
		flags = bit.bor(0, spell_flags.eval),
	},
	[8949] = {
		direct = {
			min = 137,
			max = 167,
			school1 = schools.arcane,
			coef = 1,
			per_lvl = 1.6,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 135,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 26,
		lvl_max = 32,
		lvl_outdated = 33,
		base_id = 2912,
		gcd = 1.5000001,
		train = 4500,
		flags = bit.bor(0, spell_flags.eval),
	},
	[9912] = {
		direct = {
			min = 236,
			max = 264,
			school1 = schools.nature,
			coef = 0.571,
			per_lvl = 2.1,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 2,
		cost = 180,
		power_type = powers.mana,
		rank = 8,
		lvl_req = 54,
		lvl_max = 60,
		lvl_outdated = 60,
		base_id = 5176,
		gcd = 1.5000001,
		train = 28000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[8928] = {
		direct = {
			min = 81,
			max = 97,
			school1 = schools.arcane,
			coef = 0.15,
			per_lvl = 2.1,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 41,
			max = 41,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.arcane,
			coef = 0.13,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 190,
		power_type = powers.mana,
		rank = 6,
		lvl_req = 34,
		lvl_max = 39,
		lvl_outdated = 39,
		base_id = 8921,
		gcd = 1.5000001,
		train = 10000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[17401] = {
		periodic = {
			min = 100,
			max = 100,
			tick_time = 1,
			dur = 10,
			school1 = schools.nature,
			coef = 0.129,
			per_lvl = 0.2,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit),
		},
		cast_time = 0,
		cost = 1180,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 50,
		lvl_max = 56,
		lvl_outdated = 59,
		base_id = 16914,
		gcd = 1.5000001,
		train = 23000,
		flags = bit.bor(0, spell_flags.channel, spell_flags.cd, spell_flags.eval, spell_flags.ot_haste_improved),
	},
	[24248] = {
		direct = {
			min = 57,
			max = 123,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 169,
			jump_amp = 4.1,
			flags = bit.bor(0, comp_flags.jump_amp_as_per_extra_power),
		},
		cast_time = 0,
		cost = 35,
		power_type = powers.energy,
		rank = 6,
		lvl_req = 63,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 22568,
		gcd = 1,
		train = 48000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[26994] = {
		cast_time = 2,
		cost = 0.68,
		power_type = powers.mana,
		rank = 6,
		lvl_req = 69,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 20484,
		gcd = 1.5000001,
		train = 90000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.base_mana_cost),
	},
	[9904] = {
		direct = {
			min = 55,
			max = 55,
			school1 = schools.physical,
			coef = 0,
			coef_ap_min = 0.06,
			coef_ap_max = 0.06,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 25,
			max = 25,
			tick_time = 3.0000002,
			dur = 9,
			school1 = schools.physical,
			coef = 0,
			coef_ap_min = 0.06,
			coef_ap_max = 0.06,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 40,
		power_type = powers.energy,
		rank = 4,
		lvl_req = 54,
		lvl_max = 70,
		lvl_outdated = 63,
		base_id = 1822,
		gcd = 1,
		train = 28000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[339] = {
		periodic = {
			min = 5,
			max = 5,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.055,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 1.5000001,
		cost = 50,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 8,
		lvl_max = 70,
		lvl_outdated = 17,
		base_id = 339,
		gcd = 1.5000001,
		train = 200,
		flags = bit.bor(0, spell_flags.binary, spell_flags.eval),
	},
	[22829] = {
		direct = {
			min = 45,
			max = 95,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 128,
			jump_amp = 2.5,
			flags = bit.bor(0, comp_flags.jump_amp_as_per_extra_power),
		},
		cast_time = 0,
		cost = 35,
		power_type = powers.energy,
		rank = 4,
		lvl_req = 56,
		lvl_max = 70,
		lvl_outdated = 59,
		base_id = 22568,
		gcd = 1,
		train = 30000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[9833] = {
		direct = {
			min = 130,
			max = 154,
			school1 = schools.arcane,
			coef = 0.15,
			per_lvl = 2.7,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 66,
			max = 66,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.arcane,
			coef = 0.13,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 280,
		power_type = powers.mana,
		rank = 8,
		lvl_req = 46,
		lvl_max = 51,
		lvl_outdated = 51,
		base_id = 8921,
		gcd = 1.5000001,
		train = 20000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[9852] = {
		periodic = {
			min = 25,
			max = 25,
			tick_time = 3.0000002,
			dur = 24.000002,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 1.5000001,
		cost = 110,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 48,
		lvl_max = 70,
		lvl_outdated = 57,
		base_id = 339,
		gcd = 1.5000001,
		train = 22000,
		flags = bit.bor(0, spell_flags.binary, spell_flags.eval),
	},
	[17390] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 30,
		lvl_max = 70,
		lvl_outdated = 41,
		base_id = 16857,
		gcd = 1,
		train = 300,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[9910] = {
		cast_time = 0,
		cost = 320,
		power_type = powers.mana,
		rank = 6,
		lvl_req = 54,
		lvl_max = 70,
		lvl_outdated = 63,
		base_id = 467,
		gcd = 1.5000001,
		train = 28000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[5176] = {
		direct = {
			min = 12,
			max = 14,
			school1 = schools.nature,
			coef = 0.123,
			per_lvl = 0.4,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 1.5000001,
		cost = 20,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 1,
		lvl_max = 5,
		lvl_outdated = 5,
		base_id = 5176,
		gcd = 1.5000001,
		train = -1,
		flags = bit.bor(0, spell_flags.eval),
	},
	[6798] = {
		cast_time = 0,
		cost = 10,
		power_type = powers.rage,
		rank = 2,
		lvl_req = 30,
		lvl_max = 70,
		lvl_outdated = 45,
		base_id = 5211,
		gcd = 1.5000001,
		train = 6000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.binary, spell_flags.refund_on_miss),
	},
	[780] = {
		direct = {
			min = 14,
			max = 14,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jumps = 2,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.native_jumps),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.rage,
		rank = 2,
		lvl_req = 24,
		lvl_max = 70,
		lvl_outdated = 33,
		base_id = 779,
		gcd = 1.5000001,
		train = 4000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[25298] = {
		direct = {
			min = 496,
			max = 584,
			school1 = schools.arcane,
			coef = 1,
			per_lvl = 3.1,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 340,
		power_type = powers.mana,
		rank = 7,
		lvl_req = 60,
		lvl_max = 66,
		lvl_outdated = 66,
		base_id = 2912,
		gcd = 1.5000001,
		train = 34000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[5209] = {
		cast_time = 0,
		cost = 15,
		power_type = powers.rage,
		rank = 0,
		lvl_req = 28,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 5209,
		gcd = 1.5000001,
		train = 5000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant),
	},
	[9493] = {
		periodic = {
			min = 6,
			max = 6,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 9,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 30,
		power_type = powers.energy,
		rank = 3,
		lvl_req = 36,
		lvl_max = 70,
		lvl_outdated = 43,
		base_id = 1079,
		gcd = 1,
		train = 11000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[6756] = {
		cast_time = 0,
		cost = 100,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 20,
		lvl_max = 70,
		lvl_outdated = 29,
		base_id = 1126,
		gcd = 1.5000001,
		train = 2000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[18657] = {
		cast_time = 1.5000001,
		cost = 120,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 38,
		lvl_max = 70,
		lvl_outdated = 57,
		base_id = 2637,
		gcd = 1.5000001,
		train = 12000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.binary),
	},
	[1126] = {
		cast_time = 0,
		cost = 20,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 1,
		lvl_max = 70,
		lvl_outdated = 9,
		base_id = 1126,
		gcd = 1.5000001,
		train = 10,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[1079] = {
		periodic = {
			min = 3,
			max = 3,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 4,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 30,
		power_type = powers.energy,
		rank = 1,
		lvl_req = 20,
		lvl_max = 70,
		lvl_outdated = 27,
		base_id = 1079,
		gcd = 1,
		train = 2000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[8936] = {
		direct = {
			min = 84,
			max = 98,
			school1 = schools.nature,
			coef = 0.2,
			per_lvl = 1.8,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		periodic = {
			min = 14,
			max = 14,
			tick_time = 3.0000002,
			dur = 21.000002,
			school1 = schools.nature,
			coef = 0.07,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 2,
		cost = 80,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 12,
		lvl_max = 17,
		lvl_outdated = 17,
		base_id = 8936,
		gcd = 1.5000001,
		train = 800,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[8910] = {
		periodic = {
			min = 97,
			max = 97,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.2,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 195,
		power_type = powers.mana,
		rank = 7,
		lvl_req = 40,
		lvl_max = 45,
		lvl_outdated = 45,
		base_id = 774,
		gcd = 1.5000001,
		train = 14000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[26981] = {
		periodic = {
			min = 233,
			max = 233,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.2,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 370,
		power_type = powers.mana,
		rank = 12,
		lvl_req = 63,
		lvl_max = 68,
		lvl_outdated = 68,
		base_id = 774,
		gcd = 1.5000001,
		train = 48000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[17392] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 54,
		lvl_max = 70,
		lvl_outdated = 65,
		base_id = 16857,
		gcd = 1,
		train = 1400,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[9907] = {
		direct = {
			threat_mod_flat = 0,
			school1 = schools.nature,
			flags = bit.bor(0),
		},
		cast_time = 0,
		cost = 115,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 54,
		lvl_max = 70,
		lvl_outdated = 65,
		base_id = 770,
		gcd = 1.5000001,
		train = 28000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant, spell_flags.only_threat),
	},
	[16810] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 18,
		lvl_max = 70,
		lvl_outdated = 27,
		base_id = 16689,
		gcd = 1.5000001,
		train = 95,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[5487] = {
		cast_time = 0,
		cost = 0.35,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 10,
		lvl_max = 40,
		lvl_outdated = 70,
		base_id = 5487,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.instant),
	},
	[2091] = {
		periodic = {
			min = 61,
			max = 61,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.2,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 135,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 28,
		lvl_max = 33,
		lvl_outdated = 33,
		base_id = 774,
		gcd = 1.5000001,
		train = 5000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[9000] = {
		direct = {
			threat_mod_flat = -390,
			school1 = schools.physical,
			flags = bit.bor(0, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.energy,
		rank = 2,
		lvl_req = 40,
		lvl_max = 50,
		lvl_outdated = 51,
		base_id = 8998,
		gcd = 1,
		train = 14000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.refund_on_miss, spell_flags.only_threat),
	},
	[8938] = {
		direct = {
			min = 164,
			max = 188,
			school1 = schools.nature,
			coef = 0.264,
			per_lvl = 2.5,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		periodic = {
			min = 25,
			max = 25,
			tick_time = 3.0000002,
			dur = 21.000002,
			school1 = schools.nature,
			coef = 0.093,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 2,
		cost = 135,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 18,
		lvl_max = 23,
		lvl_outdated = 23,
		base_id = 8936,
		gcd = 1.5000001,
		train = 1900,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[26993] = {
		direct = {
			threat_mod_flat = 0,
			school1 = schools.nature,
			flags = bit.bor(0),
		},
		cast_time = 0,
		cost = 145,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 66,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 770,
		gcd = 1.5000001,
		train = 34000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant, spell_flags.only_threat),
	},
	[16864] = {
		cast_time = 0,
		cost = 120,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 20,
		lvl_max = 26,
		lvl_outdated = 70,
		base_id = 16864,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant, spell_flags.talent),
	},
	[26991] = {
		cast_time = 0,
		cost = 1515,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 70,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 21849,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[8924] = {
		direct = {
			min = 13,
			max = 17,
			school1 = schools.arcane,
			coef = 0.094,
			per_lvl = 0.8,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 8,
			max = 8,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.arcane,
			coef = 0.081,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 50,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 10,
		lvl_max = 15,
		lvl_outdated = 15,
		base_id = 8921,
		gcd = 1.5000001,
		train = 300,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[17329] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 6,
		lvl_req = 58,
		lvl_max = 70,
		lvl_outdated = 67,
		base_id = 16689,
		gcd = 1.5000001,
		train = 1600,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[20719] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 40,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 20719,
		gcd = 0,
		train = 14000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.instant),
	},
	[9849] = {
		direct = {
			base_min = 88,
			base_max = 88,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 45,
		power_type = powers.energy,
		rank = 4,
		lvl_req = 48,
		lvl_max = 70,
		lvl_outdated = 57,
		base_id = 1082,
		gcd = 1,
		train = 22000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[6793] = {
		periodic = {
			min = 0,
			max = 0,
			tick_time = 0,
			dur = 6.0000005,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.no_attack, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 30,
		power_type = powers.energy,
		rank = 2,
		lvl_req = 36,
		lvl_max = 70,
		lvl_outdated = 47,
		base_id = 5217,
		gcd = 0,
		train = 11000,
		alias = 6603,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.eval, spell_flags.alias),
	},
	[22812] = {
		cast_time = 0,
		cost = 0.12,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 44,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 22812,
		gcd = 1.5000001,
		train = 18000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.base_mana_cost, spell_flags.instant),
	},
	[6809] = {
		direct = {
			base_min = 37,
			base_max = 37,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 15,
		power_type = powers.rage,
		rank = 3,
		lvl_req = 26,
		lvl_max = 70,
		lvl_outdated = 33,
		base_id = 6807,
		gcd = 0,
		train = 4500,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.on_next_attack, spell_flags.uses_attack_speed, spell_flags.refund_on_miss),
	},
	[17116] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 1,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 17116,
		gcd = 0,
		train = 0,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[8925] = {
		direct = {
			min = 25,
			max = 31,
			school1 = schools.arcane,
			coef = 0.128,
			per_lvl = 1.1,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 13,
			max = 13,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.arcane,
			coef = 0.111,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 75,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 16,
		lvl_max = 21,
		lvl_outdated = 21,
		base_id = 8921,
		gcd = 1.5000001,
		train = 1800,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[27009] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 7,
		lvl_req = 68,
		lvl_max = 77,
		lvl_outdated = 70,
		base_id = 16689,
		gcd = 1.5000001,
		train = 1700,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[31018] = {
		direct = {
			min = 52,
			max = 112,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 147,
			jump_amp = 2.7,
			flags = bit.bor(0, comp_flags.jump_amp_as_per_extra_power),
		},
		cast_time = 0,
		cost = 35,
		power_type = powers.energy,
		rank = 5,
		lvl_req = 60,
		lvl_max = 70,
		lvl_outdated = 62,
		base_id = 22568,
		gcd = 1,
		train = 30000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[8918] = {
		periodic = {
			min = 515,
			max = 515,
			tick_time = 2,
			dur = 8,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 2.6,
			per_lvl_sq = 0,
			jumps = 4,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit),
		},
		cast_time = 0,
		cost = 705,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 40,
		lvl_max = 46,
		lvl_outdated = 49,
		base_id = 740,
		gcd = 1.5000001,
		train = 14000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.channel, spell_flags.cd, spell_flags.eval, spell_flags.ot_haste_improved),
	},
	[22842] = {
		periodic = {
			min = 10,
			max = 10,
			tick_time = 1,
			dur = 10,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.no_attack, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 36,
		lvl_max = 70,
		lvl_outdated = 45,
		base_id = 22842,
		gcd = 1.5000001,
		train = 11000,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.eval),
	},
	[2912] = {
		direct = {
			min = 89,
			max = 109,
			school1 = schools.arcane,
			coef = 1,
			per_lvl = 1.2,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 95,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 20,
		lvl_max = 25,
		lvl_outdated = 25,
		base_id = 2912,
		gcd = 1.5000001,
		train = 2000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[8903] = {
		direct = {
			min = 936,
			max = 1120,
			school1 = schools.nature,
			coef = 1,
			per_lvl = 4.5,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 405,
		power_type = powers.mana,
		rank = 7,
		lvl_req = 38,
		lvl_max = 43,
		lvl_outdated = 43,
		base_id = 5185,
		gcd = 1.5000001,
		train = 12000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[18658] = {
		cast_time = 1.5000001,
		cost = 150,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 58,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 2637,
		gcd = 1.5000001,
		train = 32000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.binary),
	},
	[6795] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 10,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 6795,
		gcd = 0,
		train = 0,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant),
	},
	[9892] = {
		direct = {
			threat_mod_flat = -600,
			school1 = schools.physical,
			flags = bit.bor(0, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.energy,
		rank = 3,
		lvl_req = 52,
		lvl_max = 62,
		lvl_outdated = 59,
		base_id = 8998,
		gcd = 1,
		train = 26000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.refund_on_miss, spell_flags.only_threat),
	},
	[9885] = {
		cast_time = 0,
		cost = 445,
		power_type = powers.mana,
		rank = 7,
		lvl_req = 60,
		lvl_max = 70,
		lvl_outdated = 69,
		base_id = 1126,
		gcd = 1.5000001,
		train = 34000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[16812] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 38,
		lvl_max = 70,
		lvl_outdated = 47,
		base_id = 16689,
		gcd = 1.5000001,
		train = 600,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[99] = {
		direct = {
			threat_mod_flat = 0,
			school1 = schools.physical,
			flags = bit.bor(0, comp_flags.unbounded_aoe, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 10,
		power_type = powers.rage,
		rank = 1,
		lvl_req = 10,
		lvl_max = 20,
		lvl_outdated = 19,
		base_id = 99,
		gcd = 1.5000001,
		train = 300,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.instant, spell_flags.only_threat),
	},
	[9853] = {
		periodic = {
			min = 30,
			max = 30,
			tick_time = 3.0000002,
			dur = 27.000002,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 1.5000001,
		cost = 125,
		power_type = powers.mana,
		rank = 6,
		lvl_req = 58,
		lvl_max = 70,
		lvl_outdated = 67,
		base_id = 339,
		gcd = 1.5000001,
		train = 32000,
		flags = bit.bor(0, spell_flags.binary, spell_flags.eval),
	},
	[16689] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 10,
		lvl_max = 70,
		lvl_outdated = 17,
		base_id = 16689,
		gcd = 1.5000001,
		train = -1,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[9875] = {
		direct = {
			min = 362,
			max = 428,
			school1 = schools.arcane,
			coef = 1,
			per_lvl = 2.7,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 275,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 50,
		lvl_max = 56,
		lvl_outdated = 57,
		base_id = 2912,
		gcd = 1.5000001,
		train = 23000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[22828] = {
		direct = {
			min = 30,
			max = 70,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 92,
			jump_amp = 2,
			flags = bit.bor(0, comp_flags.jump_amp_as_per_extra_power),
		},
		cast_time = 0,
		cost = 35,
		power_type = powers.energy,
		rank = 3,
		lvl_req = 48,
		lvl_max = 70,
		lvl_outdated = 55,
		base_id = 22568,
		gcd = 1,
		train = 22000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[20739] = {
		cast_time = 2,
		cost = 0.68,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 30,
		lvl_max = 70,
		lvl_outdated = 39,
		base_id = 20484,
		gcd = 1.5000001,
		train = 6000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.base_mana_cost),
	},
	[9745] = {
		direct = {
			base_min = 71,
			base_max = 71,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 15,
		power_type = powers.rage,
		rank = 5,
		lvl_req = 42,
		lvl_max = 70,
		lvl_outdated = 49,
		base_id = 6807,
		gcd = 0,
		train = 16000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.on_next_attack, spell_flags.uses_attack_speed, spell_flags.refund_on_miss),
	},
	[5221] = {
		direct = {
			base_min = 24,
			base_max = 24,
			min = 2.25,
			max = 2.25,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 60,
		power_type = powers.energy,
		rank = 1,
		lvl_req = 22,
		lvl_max = 70,
		lvl_outdated = 29,
		base_id = 5221,
		gcd = 1,
		train = 3000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss, spell_flags.behind_target),
	},
	[6785] = {
		direct = {
			base_min = 42,
			base_max = 42,
			min = 3.85,
			max = 3.85,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.no_active_defense, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 60,
		power_type = powers.energy,
		rank = 1,
		lvl_req = 32,
		lvl_max = 70,
		lvl_outdated = 41,
		base_id = 6785,
		gcd = 1,
		train = 8000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[5195] = {
		periodic = {
			min = 15,
			max = 15,
			tick_time = 3.0000002,
			dur = 18,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 1.5000001,
		cost = 80,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 28,
		lvl_max = 70,
		lvl_outdated = 37,
		base_id = 339,
		gcd = 1.5000001,
		train = 5000,
		flags = bit.bor(0, spell_flags.binary, spell_flags.eval),
	},
	[9913] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 60,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 5215,
		gcd = 0,
		train = 34000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.binary),
	},
	[5201] = {
		direct = {
			base_min = 57,
			base_max = 57,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 45,
		power_type = powers.energy,
		rank = 3,
		lvl_req = 38,
		lvl_max = 70,
		lvl_outdated = 47,
		base_id = 1082,
		gcd = 1,
		train = 12000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[8907] = {
		cast_time = 0,
		cost = 240,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 40,
		lvl_max = 70,
		lvl_outdated = 49,
		base_id = 1126,
		gcd = 1.5000001,
		train = 14000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[24977] = {
		periodic = {
			min = 99,
			max = 99,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.127,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic, comp_flags.dot_resi_pen),
		},
		cast_time = 0,
		cost = 155,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 60,
		lvl_max = 69,
		lvl_outdated = 69,
		base_id = 5570,
		gcd = 1.5000001,
		train = 1700,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.talent),
	},
	[9856] = {
		direct = {
			min = 646,
			max = 724,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 5.3,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		periodic = {
			min = 98,
			max = 98,
			tick_time = 3.0000002,
			dur = 21.000002,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 2,
		cost = 405,
		power_type = powers.mana,
		rank = 7,
		lvl_req = 48,
		lvl_max = 53,
		lvl_outdated = 53,
		base_id = 8936,
		gcd = 1.5000001,
		train = 22000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[5217] = {
		periodic = {
			min = 0,
			max = 0,
			tick_time = 0,
			dur = 6.0000005,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.no_attack, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 30,
		power_type = powers.energy,
		rank = 1,
		lvl_req = 24,
		lvl_max = 70,
		lvl_outdated = 35,
		base_id = 5217,
		gcd = 0,
		train = 4000,
		alias = 6603,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.eval, spell_flags.alias),
	},
	[5185] = {
		direct = {
			min = 37,
			max = 51,
			school1 = schools.nature,
			coef = 0.123,
			per_lvl = 0.8,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 1.5000001,
		cost = 25,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 1,
		lvl_max = 5,
		lvl_outdated = 7,
		base_id = 5185,
		gcd = 1.5000001,
		train = -1,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[9829] = {
		direct = {
			base_min = 64,
			base_max = 64,
			min = 2.25,
			max = 2.25,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 60,
		power_type = powers.energy,
		rank = 4,
		lvl_req = 46,
		lvl_max = 70,
		lvl_outdated = 53,
		base_id = 5221,
		gcd = 1,
		train = 20000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss, spell_flags.behind_target),
	},
	[9889] = {
		direct = {
			min = 1890,
			max = 2230,
			school1 = schools.nature,
			coef = 1,
			per_lvl = 6.6,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 720,
		power_type = powers.mana,
		rank = 10,
		lvl_req = 56,
		lvl_max = 61,
		lvl_outdated = 59,
		base_id = 5185,
		gcd = 1.5000001,
		train = 30000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[8905] = {
		direct = {
			min = 188,
			max = 210,
			school1 = schools.nature,
			coef = 0.571,
			per_lvl = 1.8,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 2,
		cost = 155,
		power_type = powers.mana,
		rank = 7,
		lvl_req = 46,
		lvl_max = 52,
		lvl_outdated = 53,
		base_id = 5176,
		gcd = 1.5000001,
		train = 20000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[8927] = {
		direct = {
			min = 61,
			max = 73,
			school1 = schools.arcane,
			coef = 0.15,
			per_lvl = 1.8,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 31,
			max = 31,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.arcane,
			coef = 0.13,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 150,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 28,
		lvl_max = 33,
		lvl_outdated = 33,
		base_id = 8921,
		gcd = 1.5000001,
		train = 5000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[20747] = {
		cast_time = 2,
		cost = 0.68,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 50,
		lvl_max = 70,
		lvl_outdated = 59,
		base_id = 20484,
		gcd = 1.5000001,
		train = 23000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.base_mana_cost),
	},
	[26985] = {
		direct = {
			min = 381,
			max = 429,
			school1 = schools.nature,
			coef = 0.571,
			per_lvl = 2.7,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 2,
		cost = 255,
		power_type = powers.mana,
		rank = 10,
		lvl_req = 69,
		lvl_max = 75,
		lvl_outdated = 70,
		base_id = 5176,
		gcd = 1.5000001,
		train = 90000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[22568] = {
		direct = {
			min = 14,
			max = 30,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 36,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.jump_amp_as_per_extra_power),
		},
		cast_time = 0,
		cost = 35,
		power_type = powers.energy,
		rank = 1,
		lvl_req = 32,
		lvl_max = 70,
		lvl_outdated = 39,
		base_id = 22568,
		gcd = 1,
		train = 8000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[26979] = {
		direct = {
			min = 2707,
			max = 3197,
			school1 = schools.nature,
			coef = 1,
			per_lvl = 8.2,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 935,
		power_type = powers.mana,
		rank = 13,
		lvl_req = 69,
		lvl_max = 74,
		lvl_outdated = 70,
		base_id = 5185,
		gcd = 1.5000001,
		train = 90000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[26982] = {
		periodic = {
			min = 265,
			max = 265,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.2,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 415,
		power_type = powers.mana,
		rank = 13,
		lvl_req = 69,
		lvl_max = 74,
		lvl_outdated = 70,
		base_id = 774,
		gcd = 1.5000001,
		train = 90000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[33831] = {
		cast_time = 0,
		cost = 0.12,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 50,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 33831,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.base_mana_cost, spell_flags.instant, spell_flags.talent),
	},
	[9894] = {
		periodic = {
			min = 12,
			max = 12,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 20,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 30,
		power_type = powers.energy,
		rank = 5,
		lvl_req = 52,
		lvl_max = 70,
		lvl_outdated = 59,
		base_id = 1079,
		gcd = 1,
		train = 26000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[9754] = {
		direct = {
			min = 41,
			max = 41,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jumps = 2,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.native_jumps),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.rage,
		rank = 4,
		lvl_req = 44,
		lvl_max = 70,
		lvl_outdated = 53,
		base_id = 779,
		gcd = 1.5000001,
		train = 18000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[9758] = {
		direct = {
			min = 1199,
			max = 1427,
			school1 = schools.nature,
			coef = 1,
			per_lvl = 5.2,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 495,
		power_type = powers.mana,
		rank = 8,
		lvl_req = 44,
		lvl_max = 49,
		lvl_outdated = 49,
		base_id = 5185,
		gcd = 1.5000001,
		train = 18000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[18562] = {
		direct = {
			min = 1,
			max = 1,
			school1 = schools.nature,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 0,
		cost = 0.16,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 1,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 18562,
		gcd = 1.5000001,
		train = 0,
		alias = 774,
		flags = bit.bor(0, spell_flags.heal, spell_flags.cd, spell_flags.base_mana_cost, spell_flags.instant, spell_flags.eval, spell_flags.talent, spell_flags.alias),
	},
	[779] = {
		direct = {
			min = 10,
			max = 10,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jumps = 2,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.native_jumps),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.rage,
		rank = 1,
		lvl_req = 16,
		lvl_max = 70,
		lvl_outdated = 23,
		base_id = 779,
		gcd = 1.5000001,
		train = 1800,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[2893] = {
		cast_time = 0,
		cost = 0.13,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 26,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 2893,
		gcd = 1.5000001,
		train = 4500,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.instant),
	},
	[5186] = {
		direct = {
			min = 88,
			max = 112,
			school1 = schools.nature,
			coef = 0.314,
			per_lvl = 1.3,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 2,
		cost = 55,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 8,
		lvl_max = 13,
		lvl_outdated = 13,
		base_id = 5185,
		gcd = 1.5000001,
		train = 200,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[26978] = {
		direct = {
			min = 2364,
			max = 2790,
			school1 = schools.nature,
			coef = 1,
			per_lvl = 7.4,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 820,
		power_type = powers.mana,
		rank = 12,
		lvl_req = 62,
		lvl_max = 67,
		lvl_outdated = 68,
		base_id = 5185,
		gcd = 1.5000001,
		train = 43000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[740] = {
		periodic = {
			min = 351,
			max = 351,
			tick_time = 2,
			dur = 8,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 2.2,
			per_lvl_sq = 0,
			jumps = 4,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit),
		},
		cast_time = 0,
		cost = 525,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 30,
		lvl_max = 36,
		lvl_outdated = 39,
		base_id = 740,
		gcd = 1.5000001,
		train = 6000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.channel, spell_flags.cd, spell_flags.eval, spell_flags.ot_haste_improved),
	},
	[26988] = {
		direct = {
			min = 305,
			max = 357,
			school1 = schools.arcane,
			coef = 0.15,
			per_lvl = 3.9,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 150,
			max = 150,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.arcane,
			coef = 0.13,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 495,
		power_type = powers.mana,
		rank = 12,
		lvl_req = 70,
		lvl_max = 75,
		lvl_outdated = 70,
		base_id = 8921,
		gcd = 1.5000001,
		train = 100000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[9866] = {
		direct = {
			base_min = 78,
			base_max = 78,
			min = 3.85,
			max = 3.85,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.no_active_defense, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 60,
		power_type = powers.energy,
		rank = 3,
		lvl_req = 50,
		lvl_max = 70,
		lvl_outdated = 57,
		base_id = 6785,
		gcd = 1,
		train = 23000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[27002] = {
		direct = {
			base_min = 180,
			base_max = 180,
			min = 2.25,
			max = 2.25,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 60,
		power_type = powers.energy,
		rank = 7,
		lvl_req = 70,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 5221,
		gcd = 1,
		train = 100000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss, spell_flags.behind_target),
	},
	[782] = {
		cast_time = 0,
		cost = 60,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 14,
		lvl_max = 70,
		lvl_outdated = 23,
		base_id = 467,
		gcd = 1.5000001,
		train = 900,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[8946] = {
		cast_time = 0,
		cost = 0.13,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 14,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 8946,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.instant),
	},
	[20484] = {
		cast_time = 2,
		cost = 0.68,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 20,
		lvl_max = 70,
		lvl_outdated = 29,
		base_id = 20484,
		gcd = 1.5000001,
		train = 2000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.base_mana_cost),
	},
	[27003] = {
		direct = {
			min = 78,
			max = 78,
			school1 = schools.physical,
			coef = 0,
			coef_ap_min = 0.06,
			coef_ap_max = 0.06,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 36,
			max = 36,
			tick_time = 3.0000002,
			dur = 9,
			school1 = schools.physical,
			coef = 0,
			coef_ap_min = 0.06,
			coef_ap_max = 0.06,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 40,
		power_type = powers.energy,
		rank = 5,
		lvl_req = 64,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 1822,
		gcd = 1,
		train = 53000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[25299] = {
		periodic = {
			min = 222,
			max = 222,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.2,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 360,
		power_type = powers.mana,
		rank = 11,
		lvl_req = 60,
		lvl_max = 65,
		lvl_outdated = 62,
		base_id = 774,
		gcd = 1.5000001,
		train = 34000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[5177] = {
		direct = {
			min = 25,
			max = 29,
			school1 = schools.nature,
			coef = 0.231,
			per_lvl = 0.6,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 1.7,
		cost = 35,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 6,
		lvl_max = 12,
		lvl_outdated = 13,
		base_id = 5176,
		gcd = 1.5000001,
		train = 100,
		flags = bit.bor(0, spell_flags.eval),
	},
	[26984] = {
		direct = {
			min = 278,
			max = 312,
			school1 = schools.nature,
			coef = 0.571,
			per_lvl = 2.4,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 2,
		cost = 210,
		power_type = powers.mana,
		rank = 9,
		lvl_req = 61,
		lvl_max = 67,
		lvl_outdated = 68,
		base_id = 5176,
		gcd = 1.5000001,
		train = 39000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[27008] = {
		periodic = {
			min = 24,
			max = 24,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 47,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 30,
		power_type = powers.energy,
		rank = 7,
		lvl_req = 67,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 1079,
		gcd = 1,
		train = 73000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[6542] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 60,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 6542,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant),
	},
	[26992] = {
		cast_time = 0,
		cost = 400,
		power_type = powers.mana,
		rank = 7,
		lvl_req = 64,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 467,
		gcd = 1.5000001,
		train = 53000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[33357] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 65,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 1850,
		gcd = 0,
		train = 59000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant),
	},
	[33986] = {
		direct = {
			base_min = 105,
			base_max = 105,
			min = 1.15,
			max = 1.15,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.rage,
		rank = 2,
		lvl_req = 58,
		lvl_max = 70,
		lvl_outdated = 67,
		base_id = 33878,
		gcd = 1.5000001,
		train = 1700,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[5215] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 20,
		lvl_max = 39,
		lvl_outdated = 39,
		base_id = 5215,
		gcd = 0,
		train = 2000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.binary),
	},
	[27005] = {
		direct = {
			base_min = 147,
			base_max = 147,
			min = 3.85,
			max = 3.85,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.no_active_defense, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 60,
		power_type = powers.energy,
		rank = 5,
		lvl_req = 66,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 6785,
		gcd = 1,
		train = 66000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[5187] = {
		direct = {
			min = 195,
			max = 243,
			school1 = schools.nature,
			coef = 0.554,
			per_lvl = 1.9,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 2.5,
		cost = 110,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 14,
		lvl_max = 19,
		lvl_outdated = 19,
		base_id = 5185,
		gcd = 1.5000001,
		train = 900,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[26997] = {
		direct = {
			min = 84,
			max = 84,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jumps = 2,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.native_jumps),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.rage,
		rank = 6,
		lvl_req = 64,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 779,
		gcd = 1.5000001,
		train = 53000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[9862] = {
		periodic = {
			min = 765,
			max = 765,
			tick_time = 2,
			dur = 8,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 3.4,
			per_lvl_sq = 0,
			jumps = 4,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit),
		},
		cast_time = 0,
		cost = 975,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 50,
		lvl_max = 56,
		lvl_outdated = 59,
		base_id = 740,
		gcd = 1.5000001,
		train = 23000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.channel, spell_flags.cd, spell_flags.eval, spell_flags.ot_haste_improved),
	},
	[778] = {
		direct = {
			threat_mod_flat = 0,
			school1 = schools.nature,
			flags = bit.bor(0),
		},
		cast_time = 0,
		cost = 75,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 30,
		lvl_max = 70,
		lvl_outdated = 41,
		base_id = 770,
		gcd = 1.5000001,
		train = 6000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant, spell_flags.only_threat),
	},
	[26996] = {
		direct = {
			base_min = 176,
			base_max = 176,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 15,
		power_type = powers.rage,
		rank = 8,
		lvl_req = 67,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 6807,
		gcd = 0,
		train = 73000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.on_next_attack, spell_flags.uses_attack_speed, spell_flags.refund_on_miss),
	},
	[1823] = {
		direct = {
			min = 26,
			max = 26,
			school1 = schools.physical,
			coef = 0,
			coef_ap_min = 0.06,
			coef_ap_max = 0.06,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 15,
			max = 15,
			tick_time = 3.0000002,
			dur = 9,
			school1 = schools.physical,
			coef = 0,
			coef_ap_min = 0.06,
			coef_ap_max = 0.06,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 40,
		power_type = powers.energy,
		rank = 2,
		lvl_req = 34,
		lvl_max = 70,
		lvl_outdated = 43,
		base_id = 1822,
		gcd = 1,
		train = 10000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[6800] = {
		direct = {
			base_min = 32,
			base_max = 32,
			min = 2.25,
			max = 2.25,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 60,
		power_type = powers.energy,
		rank = 2,
		lvl_req = 30,
		lvl_max = 70,
		lvl_outdated = 37,
		base_id = 5221,
		gcd = 1,
		train = 6000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss, spell_flags.behind_target),
	},
	[5188] = {
		direct = {
			min = 363,
			max = 445,
			school1 = schools.nature,
			coef = 0.857,
			per_lvl = 2.7,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 3.0000002,
		cost = 185,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 20,
		lvl_max = 25,
		lvl_outdated = 25,
		base_id = 5185,
		gcd = 1.5000001,
		train = 2000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[9867] = {
		direct = {
			base_min = 98,
			base_max = 98,
			min = 3.85,
			max = 3.85,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.no_active_defense, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 60,
		power_type = powers.energy,
		rank = 4,
		lvl_req = 58,
		lvl_max = 70,
		lvl_outdated = 65,
		base_id = 6785,
		gcd = 1,
		train = 32000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[16811] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 28,
		lvl_max = 70,
		lvl_outdated = 37,
		base_id = 16689,
		gcd = 1.5000001,
		train = 250,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.talent),
	},
	[1430] = {
		periodic = {
			min = 29,
			max = 29,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.17,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 75,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 16,
		lvl_max = 21,
		lvl_outdated = 21,
		base_id = 774,
		gcd = 1.5000001,
		train = 1800,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[8914] = {
		cast_time = 0,
		cost = 170,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 34,
		lvl_max = 70,
		lvl_outdated = 43,
		base_id = 467,
		gcd = 1.5000001,
		train = 10000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[24974] = {
		periodic = {
			min = 32,
			max = 32,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.127,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic, comp_flags.dot_resi_pen),
		},
		cast_time = 0,
		cost = 85,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 30,
		lvl_max = 39,
		lvl_outdated = 39,
		base_id = 5570,
		gcd = 1.5000001,
		train = 300,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.talent),
	},
	[27001] = {
		direct = {
			base_min = 105,
			base_max = 105,
			min = 2.25,
			max = 2.25,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 60,
		power_type = powers.energy,
		rank = 6,
		lvl_req = 61,
		lvl_max = 70,
		lvl_outdated = 69,
		base_id = 5221,
		gcd = 1,
		train = 39000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss, spell_flags.behind_target),
	},
	[8929] = {
		direct = {
			min = 105,
			max = 125,
			school1 = schools.arcane,
			coef = 0.15,
			per_lvl = 2.4,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 53,
			max = 53,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.arcane,
			coef = 0.13,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 235,
		power_type = powers.mana,
		rank = 7,
		lvl_req = 40,
		lvl_max = 45,
		lvl_outdated = 45,
		base_id = 8921,
		gcd = 1.5000001,
		train = 14000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[8950] = {
		direct = {
			min = 201,
			max = 241,
			school1 = schools.arcane,
			coef = 1,
			per_lvl = 1.9,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 180,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 34,
		lvl_max = 40,
		lvl_outdated = 41,
		base_id = 2912,
		gcd = 1.5000001,
		train = 10000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[8921] = {
		direct = {
			min = 7,
			max = 9,
			school1 = schools.arcane,
			coef = 0.06,
			per_lvl = 0.5,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 4,
			max = 4,
			tick_time = 3.0000002,
			dur = 9,
			school1 = schools.arcane,
			coef = 0.052,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 25,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 4,
		lvl_max = 9,
		lvl_outdated = 9,
		base_id = 8921,
		gcd = 1.5000001,
		train = 100,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[5225] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 32,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 5225,
		gcd = 1.5000001,
		train = 8000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.instant),
	},
	[467] = {
		cast_time = 0,
		cost = 35,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 6,
		lvl_max = 70,
		lvl_outdated = 13,
		base_id = 467,
		gcd = 1.5000001,
		train = 100,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[5211] = {
		cast_time = 0,
		cost = 10,
		power_type = powers.rage,
		rank = 1,
		lvl_req = 14,
		lvl_max = 70,
		lvl_outdated = 29,
		base_id = 5211,
		gcd = 1.5000001,
		train = 900,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.binary, spell_flags.refund_on_miss),
	},
	[9858] = {
		direct = {
			min = 1003,
			max = 1119,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 6.8,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		periodic = {
			min = 152,
			max = 152,
			tick_time = 3.0000002,
			dur = 21.000002,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 2,
		cost = 575,
		power_type = powers.mana,
		rank = 9,
		lvl_req = 60,
		lvl_max = 65,
		lvl_outdated = 64,
		base_id = 8936,
		gcd = 1.5000001,
		train = 34000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[31709] = {
		direct = {
			threat_mod_flat = -800,
			school1 = schools.physical,
			flags = bit.bor(0, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.energy,
		rank = 4,
		lvl_req = 60,
		lvl_max = 70,
		lvl_outdated = 68,
		base_id = 8998,
		gcd = 1,
		train = 34000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.refund_on_miss, spell_flags.only_threat),
	},
	[1824] = {
		direct = {
			min = 40,
			max = 40,
			school1 = schools.physical,
			coef = 0,
			coef_ap_min = 0.06,
			coef_ap_max = 0.06,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 20,
			max = 20,
			tick_time = 3.0000002,
			dur = 9,
			school1 = schools.physical,
			coef = 0,
			coef_ap_min = 0.06,
			coef_ap_max = 0.06,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 40,
		power_type = powers.energy,
		rank = 3,
		lvl_req = 44,
		lvl_max = 70,
		lvl_outdated = 53,
		base_id = 1822,
		gcd = 1,
		train = 18000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[6807] = {
		direct = {
			base_min = 18,
			base_max = 18,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 15,
		power_type = powers.rage,
		rank = 1,
		lvl_req = 10,
		lvl_max = 70,
		lvl_outdated = 17,
		base_id = 6807,
		gcd = 0,
		train = -1,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.on_next_attack, spell_flags.uses_attack_speed, spell_flags.refund_on_miss),
	},
	[33982] = {
		direct = {
			base_min = 128,
			base_max = 128,
			min = 1.5999999,
			max = 1.5999999,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 45,
		power_type = powers.energy,
		rank = 2,
		lvl_req = 58,
		lvl_max = 70,
		lvl_outdated = 67,
		base_id = 33876,
		gcd = 1,
		train = 1700,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[33786] = {
		cast_time = 1.5000001,
		cost = 0.08,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 70,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 33786,
		gcd = 1.5000001,
		train = 100000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.binary),
	},
	[5180] = {
		direct = {
			min = 101,
			max = 115,
			school1 = schools.nature,
			coef = 0.571,
			per_lvl = 1.3,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 2,
		cost = 100,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 30,
		lvl_max = 36,
		lvl_outdated = 37,
		base_id = 5176,
		gcd = 1.5000001,
		train = 6000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[9830] = {
		direct = {
			base_min = 80,
			base_max = 80,
			min = 2.25,
			max = 2.25,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 60,
		power_type = powers.energy,
		rank = 5,
		lvl_req = 54,
		lvl_max = 70,
		lvl_outdated = 60,
		base_id = 5221,
		gcd = 1,
		train = 28000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss, spell_flags.behind_target),
	},
	[24975] = {
		periodic = {
			min = 50,
			max = 50,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.127,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic, comp_flags.dot_resi_pen),
		},
		cast_time = 0,
		cost = 110,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 40,
		lvl_max = 49,
		lvl_outdated = 49,
		base_id = 5570,
		gcd = 1.5000001,
		train = 700,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.talent),
	},
	[9752] = {
		periodic = {
			min = 9,
			max = 9,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 14,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 30,
		power_type = powers.energy,
		rank = 4,
		lvl_req = 44,
		lvl_max = 70,
		lvl_outdated = 51,
		base_id = 1079,
		gcd = 1,
		train = 18000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[9850] = {
		direct = {
			base_min = 115,
			base_max = 115,
			min = 1,
			max = 1,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh),
		},
		cast_time = 0,
		cost = 45,
		power_type = powers.energy,
		rank = 5,
		lvl_req = 58,
		lvl_max = 70,
		lvl_outdated = 66,
		base_id = 1082,
		gcd = 1,
		train = 32000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[7371] = {
		cast_time = 0,
		cost = 35,
		power_type = powers.focus,
		rank = 1,
		lvl_req = 1,
		lvl_max = 6,
		lvl_outdated = 11,
		base_id = 7371,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.pet),
	},
	[9749] = {
		direct = {
			threat_mod_flat = 0,
			school1 = schools.nature,
			flags = bit.bor(0),
		},
		cast_time = 0,
		cost = 95,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 42,
		lvl_max = 70,
		lvl_outdated = 53,
		base_id = 770,
		gcd = 1.5000001,
		train = 16000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant, spell_flags.only_threat),
	},
	[9841] = {
		periodic = {
			min = 189,
			max = 189,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.2,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 335,
		power_type = powers.mana,
		rank = 10,
		lvl_req = 58,
		lvl_max = 63,
		lvl_outdated = 59,
		base_id = 774,
		gcd = 1.5000001,
		train = 32000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.instant, spell_flags.eval),
	},
	[9863] = {
		periodic = {
			min = 1097,
			max = 1097,
			tick_time = 2,
			dur = 8,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 3.7,
			per_lvl_sq = 0,
			jumps = 4,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit),
		},
		cast_time = 0,
		cost = 1295,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 60,
		lvl_max = 66,
		lvl_outdated = 69,
		base_id = 740,
		gcd = 1.5000001,
		train = 34000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.channel, spell_flags.cd, spell_flags.eval, spell_flags.ot_haste_improved),
	},
	[8998] = {
		direct = {
			threat_mod_flat = -240,
			school1 = schools.physical,
			flags = bit.bor(0, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.energy,
		rank = 1,
		lvl_req = 28,
		lvl_max = 38,
		lvl_outdated = 39,
		base_id = 8998,
		gcd = 1,
		train = 5000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.refund_on_miss, spell_flags.only_threat),
	},
	[8941] = {
		direct = {
			min = 405,
			max = 457,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 4.1,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		periodic = {
			min = 61,
			max = 61,
			tick_time = 3.0000002,
			dur = 21.000002,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 2,
		cost = 275,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 36,
		lvl_max = 41,
		lvl_outdated = 41,
		base_id = 8936,
		gcd = 1.5000001,
		train = 11000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[9845] = {
		periodic = {
			min = 0,
			max = 0,
			tick_time = 0,
			dur = 6.0000005,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.no_attack, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 30,
		power_type = powers.energy,
		rank = 3,
		lvl_req = 48,
		lvl_max = 70,
		lvl_outdated = 59,
		base_id = 5217,
		gcd = 0,
		train = 22000,
		alias = 6603,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.eval, spell_flags.alias),
	},
	[26989] = {
		periodic = {
			min = 39,
			max = 39,
			tick_time = 3.0000002,
			dur = 27.000002,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 1.5000001,
		cost = 160,
		power_type = powers.mana,
		rank = 7,
		lvl_req = 68,
		lvl_max = 77,
		lvl_outdated = 70,
		base_id = 339,
		gcd = 1.5000001,
		train = 81000,
		flags = bit.bor(0, spell_flags.binary, spell_flags.eval),
	},
	[5570] = {
		periodic = {
			min = 18,
			max = 18,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.127,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic, comp_flags.dot_resi_pen),
		},
		cast_time = 0,
		cost = 50,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 20,
		lvl_max = 29,
		lvl_outdated = 29,
		base_id = 5570,
		gcd = 1.5000001,
		train = -1,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.talent),
	},
	[783] = {
		periodic = {
			min = -11,
			max = -11,
			tick_time = 2,
			dur = -0.001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 0.13,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 30,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 783,
		gcd = 1.5000001,
		train = 6000,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.instant, spell_flags.resource_regen),
	},
	[1822] = {
		direct = {
			min = 17,
			max = 17,
			school1 = schools.physical,
			coef = 0,
			coef_ap_min = 0.06,
			coef_ap_max = 0.06,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 10,
			max = 10,
			tick_time = 3.0000002,
			dur = 9,
			school1 = schools.physical,
			coef = 0,
			coef_ap_min = 0.06,
			coef_ap_max = 0.06,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 40,
		power_type = powers.energy,
		rank = 1,
		lvl_req = 24,
		lvl_max = 70,
		lvl_outdated = 33,
		base_id = 1822,
		gcd = 1,
		train = 4000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[9896] = {
		periodic = {
			min = 17,
			max = 17,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			per_resource = 28,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 30,
		power_type = powers.energy,
		rank = 6,
		lvl_req = 60,
		lvl_max = 70,
		lvl_outdated = 66,
		base_id = 1079,
		gcd = 1,
		train = 34000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.finishing_move_dmg),
	},
	[26986] = {
		direct = {
			min = 540,
			max = 636,
			school1 = schools.arcane,
			coef = 1,
			per_lvl = 3.4,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 370,
		power_type = powers.mana,
		rank = 8,
		lvl_req = 67,
		lvl_max = 73,
		lvl_outdated = 70,
		base_id = 2912,
		gcd = 1.5000001,
		train = 73000,
		flags = bit.bor(0, spell_flags.eval),
	},
	[26990] = {
		cast_time = 0,
		cost = 565,
		power_type = powers.mana,
		rank = 8,
		lvl_req = 70,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 1126,
		gcd = 1.5000001,
		train = 100000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[8940] = {
		direct = {
			min = 318,
			max = 360,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 3.6,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		periodic = {
			min = 49,
			max = 49,
			tick_time = 3.0000002,
			dur = 21.000002,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 2,
		cost = 230,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 30,
		lvl_max = 35,
		lvl_outdated = 35,
		base_id = 8936,
		gcd = 1.5000001,
		train = 6000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[9821] = {
		cast_time = 0,
		cost = 0,
		power_type = powers.mana,
		rank = 2,
		lvl_req = 46,
		lvl_max = 70,
		lvl_outdated = 64,
		base_id = 1850,
		gcd = 0,
		train = 20000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant),
	},
	[16914] = {
		periodic = {
			min = 70,
			max = 70,
			tick_time = 1,
			dur = 10,
			school1 = schools.nature,
			coef = 0.129,
			per_lvl = 0.2,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit),
		},
		cast_time = 0,
		cost = 880,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 40,
		lvl_max = 46,
		lvl_outdated = 49,
		base_id = 16914,
		gcd = 1.5000001,
		train = 14000,
		flags = bit.bor(0, spell_flags.channel, spell_flags.cd, spell_flags.eval, spell_flags.ot_haste_improved),
	},
	[9835] = {
		direct = {
			min = 189,
			max = 221,
			school1 = schools.arcane,
			coef = 0.15,
			per_lvl = 3.3,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 96,
			max = 96,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.arcane,
			coef = 0.13,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 375,
		power_type = powers.mana,
		rank = 10,
		lvl_req = 58,
		lvl_max = 63,
		lvl_outdated = 62,
		base_id = 8921,
		gcd = 1.5000001,
		train = 32000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[9634] = {
		cast_time = 0,
		cost = 0.35,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 40,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 9634,
		gcd = 1.5000001,
		train = 14000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.instant),
	},
	[9908] = {
		direct = {
			min = 60,
			max = 60,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jumps = 2,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.native_jumps),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.rage,
		rank = 5,
		lvl_req = 54,
		lvl_max = 70,
		lvl_outdated = 63,
		base_id = 779,
		gcd = 1.5000001,
		train = 28000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[40120] = {
		periodic = {
			min = -11,
			max = -11,
			tick_time = 2,
			dur = -0.001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 0.13,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 70,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 40120,
		gcd = 1.5000001,
		train = 0,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.instant, spell_flags.resource_regen),
	},
	[9846] = {
		periodic = {
			min = 0,
			max = 0,
			tick_time = 0,
			dur = 6.0000005,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.no_attack, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 30,
		power_type = powers.energy,
		rank = 4,
		lvl_req = 60,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 5217,
		gcd = 0,
		train = 34000,
		alias = 6603,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.eval, spell_flags.alias),
	},
	[1075] = {
		cast_time = 0,
		cost = 105,
		power_type = powers.mana,
		rank = 3,
		lvl_req = 24,
		lvl_max = 70,
		lvl_outdated = 33,
		base_id = 467,
		gcd = 1.5000001,
		train = 4000,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant),
	},
	[26980] = {
		direct = {
			min = 1215,
			max = 1355,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 7.7,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		periodic = {
			min = 182,
			max = 182,
			tick_time = 3.0000002,
			dur = 21.000002,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 2,
		cost = 675,
		power_type = powers.mana,
		rank = 10,
		lvl_req = 65,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 8936,
		gcd = 1.5000001,
		train = 59000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[2637] = {
		cast_time = 1.5000001,
		cost = 90,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 18,
		lvl_max = 70,
		lvl_outdated = 37,
		base_id = 2637,
		gcd = 1.5000001,
		train = 1900,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.binary),
	},
	[8926] = {
		direct = {
			min = 40,
			max = 48,
			school1 = schools.arcane,
			coef = 0.15,
			per_lvl = 1.4,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0),
		},
		periodic = {
			min = 20,
			max = 20,
			tick_time = 3.0000002,
			dur = 12.000001,
			school1 = schools.arcane,
			coef = 0.13,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 0,
		cost = 105,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 22,
		lvl_max = 27,
		lvl_outdated = 27,
		base_id = 8921,
		gcd = 1.5000001,
		train = 3000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval),
	},
	[5189] = {
		direct = {
			min = 572,
			max = 694,
			school1 = schools.nature,
			coef = 1,
			per_lvl = 3.5,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 270,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 26,
		lvl_max = 31,
		lvl_outdated = 31,
		base_id = 5185,
		gcd = 1.5000001,
		train = 4500,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[33983] = {
		direct = {
			base_min = 165,
			base_max = 165,
			min = 1.5999999,
			max = 1.5999999,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.applies_mh, comp_flags.weapon_pct),
		},
		cast_time = 0,
		cost = 45,
		power_type = powers.energy,
		rank = 3,
		lvl_req = 68,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 33876,
		gcd = 1,
		train = 1700,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[26983] = {
		periodic = {
			min = 1518,
			max = 1518,
			tick_time = 2,
			dur = 8,
			school1 = schools.nature,
			coef = 0.286,
			per_lvl = 4.5,
			per_lvl_sq = 0,
			jumps = 4,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0, comp_flags.cant_crit),
		},
		cast_time = 0,
		cost = 1650,
		power_type = powers.mana,
		rank = 5,
		lvl_req = 70,
		lvl_max = 76,
		lvl_outdated = 70,
		base_id = 740,
		gcd = 1.5000001,
		train = 100000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.channel, spell_flags.cd, spell_flags.eval, spell_flags.ot_haste_improved),
	},
	[25297] = {
		direct = {
			min = 2267,
			max = 2677,
			school1 = schools.nature,
			coef = 1,
			per_lvl = 7.3,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 800,
		power_type = powers.mana,
		rank = 11,
		lvl_req = 60,
		lvl_max = 65,
		lvl_outdated = 61,
		base_id = 5185,
		gcd = 1.5000001,
		train = 34000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
	[8983] = {
		cast_time = 0,
		cost = 10,
		power_type = powers.rage,
		rank = 3,
		lvl_req = 46,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 5211,
		gcd = 1.5000001,
		train = 20000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.binary, spell_flags.refund_on_miss),
	},
	[26178] = {
		direct = {
			min = 150,
			max = 150,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 35,
		power_type = powers.focus,
		rank = 3,
		lvl_req = 24,
		lvl_max = 28,
		lvl_outdated = 35,
		base_id = 7371,
		gcd = 1.5000001,
		train = 0,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.pet),
	},
	[9490] = {
		direct = {
			threat_mod_flat = 0,
			school1 = schools.physical,
			flags = bit.bor(0, comp_flags.unbounded_aoe, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 10,
		power_type = powers.rage,
		rank = 3,
		lvl_req = 32,
		lvl_max = 42,
		lvl_outdated = 41,
		base_id = 99,
		gcd = 1.5000001,
		train = 8000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.instant, spell_flags.only_threat),
	},
	[770] = {
		direct = {
			threat_mod_flat = 0,
			school1 = schools.nature,
			flags = bit.bor(0),
		},
		cast_time = 0,
		cost = 55,
		power_type = powers.mana,
		rank = 1,
		lvl_req = 18,
		lvl_max = 70,
		lvl_outdated = 29,
		base_id = 770,
		gcd = 1.5000001,
		train = 1900,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.instant, spell_flags.only_threat),
	},
	[24858] = {
		cast_time = 0,
		cost = 0.22,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 40,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 24858,
		gcd = 1.5000001,
		train = 0,
		anyschool = schools.nature,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.instant, spell_flags.talent),
	},
	[27006] = {
		periodic = {
			min = 100,
			max = 100,
			tick_time = 3.0000002,
			dur = 18,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.no_attack, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 50,
		power_type = powers.energy,
		rank = 4,
		lvl_req = 66,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 9005,
		gcd = 1,
		train = 66000,
		flags = bit.bor(0, spell_flags.instant, spell_flags.binary, spell_flags.eval, spell_flags.refund_on_miss),
	},
	[5196] = {
		periodic = {
			min = 20,
			max = 20,
			tick_time = 3.0000002,
			dur = 21.000002,
			school1 = schools.nature,
			coef = 0.1,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic),
		},
		cast_time = 1.5000001,
		cost = 95,
		power_type = powers.mana,
		rank = 4,
		lvl_req = 38,
		lvl_max = 70,
		lvl_outdated = 47,
		base_id = 339,
		gcd = 1.5000001,
		train = 12000,
		flags = bit.bor(0, spell_flags.binary, spell_flags.eval),
	},
	[27004] = {
		direct = {
			threat_mod_flat = -1170,
			school1 = schools.physical,
			flags = bit.bor(0, comp_flags.no_attack),
		},
		cast_time = 0,
		cost = 20,
		power_type = powers.energy,
		rank = 5,
		lvl_req = 69,
		lvl_max = 79,
		lvl_outdated = 70,
		base_id = 8998,
		gcd = 1,
		train = 90000,
		anyschool = schools.physical,
		flags = bit.bor(0, spell_flags.cd, spell_flags.instant, spell_flags.refund_on_miss, spell_flags.only_threat),
	},
	[1066] = {
		periodic = {
			min = -11,
			max = -11,
			tick_time = 2,
			dur = -0.001,
			school1 = schools.physical,
			coef = 0,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.ignores_mitigation, comp_flags.periodic, comp_flags.bleed),
		},
		cast_time = 0,
		cost = 0.13,
		power_type = powers.mana,
		rank = 0,
		lvl_req = 16,
		lvl_max = 70,
		lvl_outdated = 70,
		base_id = 1066,
		gcd = 1.5000001,
		train = 0,
		flags = bit.bor(0, spell_flags.base_mana_cost, spell_flags.instant, spell_flags.resource_regen),
	},
	[27013] = {
		periodic = {
			min = 132,
			max = 132,
			tick_time = 2,
			dur = 12.000001,
			school1 = schools.nature,
			coef = 0.127,
			per_lvl = 0,
			per_lvl_sq = 0,
			jump_amp = 1,
			flags = bit.bor(0, comp_flags.cant_crit, comp_flags.periodic, comp_flags.dot_resi_pen),
		},
		cast_time = 0,
		cost = 175,
		power_type = powers.mana,
		rank = 6,
		lvl_req = 70,
		lvl_max = 79,
		lvl_outdated = 70,
		base_id = 5570,
		gcd = 1.5000001,
		train = 2500,
		flags = bit.bor(0, spell_flags.instant, spell_flags.eval, spell_flags.talent),
	},
	[6778] = {
		direct = {
			min = 742,
			max = 894,
			school1 = schools.nature,
			coef = 1,
			per_lvl = 4,
			per_lvl_sq = 0,
			jump_amp = 1,
			threat_mod = -0.75,
			flags = bit.bor(0),
		},
		cast_time = 3.5000002,
		cost = 335,
		power_type = powers.mana,
		rank = 6,
		lvl_req = 32,
		lvl_max = 37,
		lvl_outdated = 37,
		base_id = 5185,
		gcd = 1.5000001,
		train = 8000,
		flags = bit.bor(0, spell_flags.heal, spell_flags.eval),
	},
};
sc.base_mana_by_lvl = {60, 66, 73, 81, 90, 100, 111, 123, 136, 150, 165, 182, 200, 219, 239, 260, 282, 305, 329, 354, 380, 392, 420, 449, 479, 509, 524, 554, 584, 614, 629, 659, 689, 704, 734, 749, 779, 809, 824, 854, 869, 899, 914, 944, 959, 989, 1004, 1019, 1049, 1064, 1079, 1109, 1124, 1139, 1154, 1169, 1199, 1214, 1229, 1244, 1357, 1469, 1582, 1694, 1807, 1919, 2032, 2145, 2257, 2370, };
sc.spell_crit_to_int = {0.1431, 0.1369, 0.1312, 0.1259, 0.12110001, 0.1166, 0.1124, 0.1124, 0.1086, 0.09839999, 0.0926, 0.0851, 0.0807, 0.075, 0.0684, 0.0656, 0.0617, 0.059400003, 0.0562, 0.0516, 0.05, 0.0477, 0.0463, 0.0437, 0.042, 0.0409, 0.0394, 0.0384, 0.0366, 0.0346, 0.0339, 0.0325, 0.0318, 0.030899998, 0.029700002, 0.0292, 0.028400002, 0.0276, 0.0269, 0.0256, 0.0252, 0.0244, 0.023999998, 0.0233, 0.0228, 0.0223, 0.0219, 0.0214, 0.0209, 0.0202, 0.0198, 0.0193, 0.0191, 0.0186, 0.0182, 0.0179, 0.0176, 0.0173, 0.0169, 0.0164, 0.0162, 0.0157, 0.014999999, 0.0146, 0.014200001, 0.0137, 0.0133, 0.0131, 0.0128, 0.0125, };
sc.physical_crit_to_agi = {0.202, 0.202, 0.1923, 0.1923, 0.1836, 0.1836, 0.1756, 0.1756, 0.1683, 0.1553, 0.1496, 0.1496, 0.1443, 0.1443, 0.1346, 0.1346, 0.1303, 0.1262, 0.1262, 0.1122, 0.1122, 0.1092, 0.1063, 0.1063, 0.101, 0.101, 0.0985, 0.0962, 0.0962, 0.0878, 0.0859, 0.0859, 0.0841, 0.0824, 0.0808, 0.0792, 0.0777, 0.0777, 0.0762, 0.0709, 0.0696, 0.0696, 0.0685, 0.0673, 0.0651, 0.0641, 0.0641, 0.0631, 0.0621, 0.0585, 0.0577, 0.056899995, 0.0561, 0.0561, 0.0546, 0.0539, 0.0531, 0.052500002, 0.0518, 0.0493, 0.0478, 0.0472, 0.0456, 0.0447, 0.0438, 0.043, 0.0424, 0.0412, 0.0406, 0.04, };
sc.dodge_to_agi = {0.34340003, 0.34340003, 0.32691002, 0.32691002, 0.31212, 0.31212, 0.29852, 0.29852, 0.28611, 0.26401, 0.25432, 0.25432, 0.24531001, 0.24531001, 0.22882001, 0.22882001, 0.22151001, 0.21454002, 0.21454002, 0.19074, 0.19074, 0.18564, 0.18071, 0.18071, 0.17170002, 0.17170002, 0.16745, 0.16353999, 0.16353999, 0.14926, 0.14603001, 0.14603001, 0.14297001, 0.14008, 0.13736, 0.13464001, 0.13209, 0.13209, 0.12954, 0.12053001, 0.11832, 0.11832, 0.116450004, 0.114410006, 0.11067001, 0.108969994, 0.108969994, 0.10727001, 0.10557, 0.09945, 0.09809001, 0.096729994, 0.09537, 0.09537, 0.09282, 0.091630004, 0.090270005, 0.089250006, 0.08806, 0.08381, 0.08126, 0.080240004, 0.07752, 0.07599, 0.07446, 0.0731, 0.07208, 0.07004, 0.06902, 0.068, };
sc.spids = {
	rebirth = 20484,
	swift_flight_form = 40120,
	dodge = 81,
	rip = 1079,
	demoralizing_roar = 99,
	mangle_bear = 33878,
	wrath = 5176,
	moonfire = 8921,
	mangle_cat = 33876,
	hibernate = 2637,
	cower = 8998,
	natures_swiftness = 17116,
	challenging_roar = 5209,
	frenzied_regeneration = 22842,
	travel_form = 783,
	moonkin_form = 24858,
	soothe_animal = 2908,
	aquatic_form = 1066,
	dire_bear_form = 9634,
	shred = 5221,
	force_of_nature = 33831,
	natures_grasp = 16689,
	growl = 6795,
	pounce = 9005,
	starfire = 2912,
	lacerate = 33745,
	attack = 6603,
	insect_swarm = 5570,
	teleport_moonglade = 18960,
	bear_form = 5487,
	feline_grace = 20719,
	maim = 22570,
	barkskin = 22812,
	abolish_poison = 2893,
	rejuvenation = 774,
	berserking = 20554,
	track_humanoids = 5225,
	enrage = 5229,
	gift_of_the_wild = 21849,
	thorns = 467,
	tigers_fury = 5217,
	dash = 1850,
	remove_curse = 2782,
	mark_of_the_wild = 1126,
	enraged_defense = 6542,
	faerie_fire_feral = 16857,
	ravage = 6785,
	prowl = 5215,
	healing_touch = 5185,
	lifebloom = 33763,
	flight_form = 33943,
	hurricane = 16914,
	entangling_roots = 339,
	regrowth = 8936,
	swiftmend = 18562,
	tranquility = 740,
	maul = 6807,
	charge = 7371,
	swipe = 779,
	omen_of_clarity = 16864,
	claw = 1082,
	faerie_fire = 770,
	feral_charge = 16979,
	tree_of_life = 33891,
	bash = 5211,
	innervate = 29166,
	cat_form = 768,
	cyclone = 33786,
	ferocious_bite = 22568,
	rake = 1822,
	cure_poison = 8946,
};
sc.rank_seqs = {
	[20484] = {20484,20739,20742,20747,20748,26994,},
	[40120] = {40120,},
	[81] = {81,},
	[1079] = {1079,9492,9493,9752,9894,9896,27008,},
	[99] = {99,1735,9490,9747,9898,26998,},
	[33878] = {33878,33986,33987,},
	[5176] = {5176,5177,5178,5179,5180,6780,8905,9912,26984,26985,},
	[8921] = {8921,8924,8925,8926,8927,8928,8929,9833,9834,9835,26987,26988,},
	[33876] = {33876,33982,33983,},
	[2637] = {2637,18657,18658,},
	[8998] = {8998,9000,9892,31709,27004,},
	[17116] = {17116,},
	[5209] = {5209,},
	[22842] = {22842,22895,22896,26999,},
	[783] = {783,},
	[24858] = {24858,},
	[2908] = {2908,8955,9901,26995,},
	[1066] = {1066,},
	[9634] = {9634,},
	[5221] = {5221,6800,8992,9829,9830,27001,27002,},
	[33831] = {33831,},
	[16689] = {16689,16810,16811,16812,16813,17329,27009,},
	[6795] = {6795,},
	[9005] = {9005,9823,9827,27006,},
	[2912] = {2912,8949,8950,8951,9875,9876,25298,26986,},
	[33745] = {33745,},
	[6603] = {6603,},
	[5570] = {5570,24974,24975,24976,24977,27013,},
	[18960] = {18960,},
	[5487] = {5487,},
	[20719] = {20719,},
	[22570] = {22570,},
	[22812] = {22812,},
	[2893] = {2893,},
	[774] = {774,1058,1430,2090,2091,3627,8910,9839,9840,9841,25299,26981,26982,},
	[20554] = {20554,},
	[5225] = {5225,},
	[5229] = {5229,},
	[21849] = {21849,21850,26991,},
	[467] = {467,782,1075,8914,9756,9910,26992,},
	[5217] = {5217,6793,9845,9846,},
	[1850] = {1850,9821,33357,},
	[2782] = {2782,},
	[1126] = {1126,5232,6756,5234,8907,9884,9885,26990,},
	[6542] = {6542,},
	[16857] = {16857,17390,17391,17392,27011,},
	[6785] = {6785,6787,9866,9867,27005,},
	[5215] = {5215,6783,9913,},
	[5185] = {5185,5186,5187,5188,5189,6778,8903,9758,9888,9889,25297,26978,26979,},
	[33763] = {33763,},
	[33943] = {33943,},
	[16914] = {16914,17401,17402,27012,},
	[339] = {339,1062,5195,5196,9852,9853,26989,},
	[8936] = {8936,8938,8939,8940,8941,9750,9856,9857,9858,26980,},
	[18562] = {18562,},
	[740] = {740,8918,9862,9863,26983,},
	[6807] = {6807,6808,6809,8972,9745,9880,9881,26996,},
	[7371] = {7371,26177,26178,26179,26201,27685,},
	[779] = {779,780,769,9754,9908,26997,},
	[16864] = {16864,},
	[1082] = {1082,3029,5201,9849,9850,27000,},
	[770] = {770,778,9749,9907,26993,},
	[16979] = {16979,},
	[33891] = {33891,},
	[5211] = {5211,6798,8983,},
	[29166] = {29166,},
	[768] = {768,},
	[33786] = {33786,},
	[22568] = {22568,22827,22828,22829,31018,24248,},
	[1822] = {1822,1823,1824,9904,27003,},
	[8946] = {8946,},
};
sc.spells_lvl_ordered = {
6603, 20554, 81, 5176, 1126, 17116, 5185, 18562, 7371, 774, 8921, 5177, 467, 339, 5186, 5232, 18960, 1058, 5487, 8924, 6795, 99, 16689, 6807, 26177, 5229, 8936, 5178, 782, 8946, 5187, 5211, 8925, 779, 1430, 1066, 6808, 1062, 16810, 8938, 2637, 770, 1082, 16979, 768, 1735, 6756, 1079, 16864, 2912, 20484, 5215, 5188, 5570, 5179, 2908, 2090, 5221, 8926, 8939, 2782, 780, 5217, 1822, 1075, 26178, 16857, 1850, 8949, 6809, 2893, 5189, 9492, 3029, 5209, 2091, 5195, 8927, 16811, 8998, 5234, 17390, 6798, 20739, 740, 778, 6800, 24974, 5180, 783, 8940, 6785, 22568, 5225, 9490, 6778, 8972, 3627, 769, 8928, 1823, 8914, 8950, 9005, 26179, 9493, 6793, 22842, 8941, 8992, 8955, 6780, 18657, 8903, 16812, 5201, 5196, 29166, 20742, 22827, 6783, 8910, 9000, 20719, 8918, 8907, 8929, 24975, 16914, 9634, 24858, 6787, 17391, 8951, 9747, 9750, 9745, 9749, 9756, 22812, 9754, 9758, 1824, 9752, 9839, 9823, 22895, 9833, 9829, 8905, 9821, 8983, 26201, 16813, 9852, 9849, 22828, 9856, 9845, 21849, 9888, 9884, 33876, 9880, 24976, 33878, 33891, 17401, 9875, 20747, 33831, 9866, 9862, 9840, 9898, 9834, 9892, 9894, 9857, 9901, 9912, 9904, 9910, 17392, 9907, 9830, 9908, 22896, 9827, 22829, 9889, 9876, 9881, 17329, 18658, 9853, 33986, 9867, 33982, 9850, 9841, 9835, 27685, 17402, 20748, 21850, 25298, 31018, 9885, 9913, 24977, 25299, 6542, 9858, 31709, 9863, 9896, 9846, 25297, 26984, 27001, 22570, 26998, 26978, 26987, 24248, 26981, 33763, 27003, 26992, 26997, 26999, 33357, 26980, 27011, 33745, 26993, 27005, 27006, 27000, 27008, 26996, 26986, 33987, 33943, 27009, 26989, 33983, 26994, 26985, 26979, 26982, 27004, 26995, 27012, 26991, 26988, 27002, 33786, 26990, 40120, 26983, 27013, };
sc.talent_order = {
	[3] = {1,2,4,3,6,9,12,7,14,10,11,8,13,16,5,20,15,18,17,19,},
	[2] = {3,2,6,4,1,12,10,5,8,9,7,11,15,16,13,18,17,14,21,19,20,},
	[1] = {2,1,14,8,21,3,5,9,4,12,7,15,10,6,11,16,17,13,18,19,20,},
};
sc.talent_effects = {
	[33890] = {
			{"ability", "coef_mod", 0.19999999, {33763,740,774,8936,}, 0, 0},
	},
	[33882] = {
			{"by_school", "crit", 0.02, {1,2,3,4,5,6,7,}, 32, 1},
	},
	[33607] = {
			{"ability", "coef_mod_flat", 0.19999999, {2912,}, 0, 0},
			{"ability", "coef_mod_flat", 0.099999994, {5176,}, 0, 1},
	},
	[16934] = {
			{"ability", "cost_mod_flat", -10, {6807,33878,779,}, 0, 0},
			{"ability", "cost_mod_flat", -1, {1822,1082,33876,}, 0, 1},
	},
	[33855] = {
			{"by_attr", "stat_mod", 0.02, {1,2,3,4,5,}, 32, 0},
			{"raw", "attacker_melee_crit", -0.02, nil, 0, 1},
	},
	[33605] = {
			{"ability", "coef_mod_flat", 0.12, {2912,}, 0, 0},
			{"ability", "coef_mod_flat", 0.06, {5176,}, 0, 1},
	},
	[17006] = {
			{"by_attr", "stat_mod", 0.16, {4,}, 32, 0},
	},
	[16938] = {
			{"ability", "cost_mod_flat", -50, {6807,33878,779,}, 0, 0},
			{"ability", "cost_mod_flat", -5, {1822,1082,33876,}, 0, 1},
	},
	[16814] = {
			{"ability", "cast_mod_flat", -0.1, {5176,2912,}, 0, 0},
	},
	[17003] = {
			{"by_attr", "stat_mod", 0.04, {4,}, 32, 0},
	},
	[17120] = {
			{"ability", "threat", -0.12, {33763,339,740,5185,8921,5176,29166,770,5570,774,16914,2912,18562,8936,}, 0, 0},
	},
	[24945] = {
			{"ability", "effect_mod", 0.08, {33763,740,5185,8936,}, 0, 0},
			{"ability", "effect_mod_ot", 0.08, {33763,740,774,8936,}, 0, 1},
	},
	[17075] = {
			{"ability", "crit", 0.19999999, {8936,}, 0, 0},
	},
	[16845] = {
			{"ability", "cost_mod", -0.03, {5185,8921,5176,774,2912,8936,}, 0, 0},
	},
	[33859] = {
			{"by_school", "crit_mod", 0.02, {1,}, 0, 0},
	},
	[17051] = {
			{"aura_pts", -1, 0.14, {21850,9885,21849,34906,9884,39233,1126,5232,26990,6756,5234,26991,24752,8907,}, 0, 0},
	},
	[17123] = {
			{"ability", "threat", -0.5, {740,}, 0, 0},
	},
	[16858] = {
			{"ability", "base_mod", 0.08, {99,}, 0, 0},
			{"ability", "base_mod_ot", 0.08, {99,}, 0, 0},
			{"ability", "effect_mod", 0.03, {22568,1079,}, 0, 1},
	},
	[33868] = {
			{"by_school", "crit_mod", 0.08, {1,}, 0, 0},
	},
	[17071] = {
			{"ability", "cast_mod_flat", -0.3, {5185,}, 0, 0},
			{"raw", "phys_mod", 0.06, nil, 1, 1},
	},
	[16999] = {
			{"ability", "effect_mod", 0.19999999, {1822,1082,}, 0, 0},
			{"ability", "effect_mod_ot", 0.19999999, {1822,1082,}, 0, 1},
			{"ability", "base_mod", 0.19999999, {33876,}, 0, 2},
	},
	[17124] = {
			{"ability", "threat", -1, {740,}, 0, 0},
	},
	[16821] = {
			{"ability", "crit", 0.049999997, {8921,}, 0, 0},
			{"ability", "effect_mod", 0.049999997, {8921,}, 0, 1},
			{"ability", "effect_mod_ot", 0.049999997, {8921,}, 0, 2},
	},
	[17004] = {
			{"by_attr", "stat_mod", 0.08, {4,}, 32, 0},
	},
	[34151] = {
			{"by_attr", "stat_mod", 0.049999997, {5,}, 32, 0},
	},
	[17111] = {
			{"ability", "effect_mod_ot", 0.049999997, {774,}, 0, 0},
	},
	[16816] = {
			{"ability", "cast_mod_flat", -0.3, {5176,2912,}, 0, 0},
	},
	[16822] = {
			{"ability", "crit", 0.099999994, {8921,}, 0, 0},
			{"ability", "effect_mod", 0.099999994, {8921,}, 0, 1},
			{"ability", "effect_mod_ot", 0.099999994, {8921,}, 0, 2},
	},
	[33599] = {
			{"raw", "mp5_from_int_mod", 0.07, nil, 0, 0},
	},
	[16835] = {
			{"ability", "cost_mod", -0.29999998, {783,33943,768,1066,40120,}, 0, 0},
	},
	[16818] = {
			{"ability", "cast_mod_flat", -0.5, {5176,2912,}, 0, 0},
	},
	[33606] = {
			{"ability", "coef_mod_flat", 0.16, {2912,}, 0, 0},
			{"ability", "coef_mod_flat", 0.08, {5176,}, 0, 1},
	},
	[16901] = {
			{"ability", "effect_mod", 0.099999994, {8921,5176,2912,}, 0, 0},
			{"ability", "effect_mod_ot", 0.099999994, {8921,}, 0, 1},
	},
	[17070] = {
			{"ability", "cast_mod_flat", -0.2, {5185,}, 0, 0},
			{"raw", "phys_mod", 0.04, nil, 1, 1},
	},
	[16998] = {
			{"ability", "effect_mod", 0.099999994, {1822,1082,}, 0, 0},
			{"ability", "effect_mod_ot", 0.099999994, {1822,1082,}, 0, 1},
			{"ability", "base_mod", 0.099999994, {33876,}, 0, 2},
	},
	[17078] = {
			{"ability", "crit", 0.5, {8936,}, 0, 0},
	},
	[16947] = {
			{"aura_pts_flat", -1, 0.049999997, {21178,}, 0, 1},
	},
	[16929] = {
			{"raw", "base_res_phys_mod", 0.04, nil, 32, 0},
	},
	[17108] = {
			{"raw", "regen_while_casting", 0.29999998, nil, 0, 0},
	},
	[17076] = {
			{"ability", "crit", 0.29999998, {8936,}, 0, 0},
	},
	[24946] = {
			{"ability", "effect_mod", 0.099999994, {33763,740,5185,8936,}, 0, 0},
			{"ability", "effect_mod_ot", 0.099999994, {33763,740,774,8936,}, 0, 1},
	},
	[17107] = {
			{"raw", "regen_while_casting", 0.19999999, nil, 0, 0},
	},
	[33596] = {
			{"by_school", "spell_hit", 0.04, {2,3,4,5,6,7,}, 0, 0},
	},
	[33873] = {
			{"by_attr", "hp_of_stat_pct", 1, {2,}, 32, 0},
	},
	[16937] = {
			{"ability", "cost_mod_flat", -40, {6807,33878,779,}, 0, 0},
			{"ability", "cost_mod_flat", -4, {1822,1082,33876,}, 0, 1},
	},
	[33853] = {
			{"by_attr", "stat_mod", 0.01, {1,2,3,4,5,}, 32, 0},
			{"raw", "attacker_melee_crit", -0.01, nil, 0, 1},
	},
	[17121] = {
			{"ability", "threat", -0.16, {33763,339,740,5185,8921,5176,29166,770,5570,774,16914,2912,18562,8936,}, 0, 0},
	},
	[33883] = {
			{"by_school", "crit", 0.03, {1,2,3,4,5,6,7,}, 32, 1},
	},
	[16861] = {
			{"ability", "base_mod", 0.32, {99,}, 0, 0},
			{"ability", "base_mod_ot", 0.32, {99,}, 0, 0},
			{"ability", "effect_mod", 0.12, {22568,1079,}, 0, 1},
	},
	[33867] = {
			{"by_school", "crit_mod", 0.06, {1,}, 0, 0},
	},
	[17053] = {
			{"aura_pts", -1, 0.21, {21850,9885,21849,34906,9884,39233,1126,5232,26990,6756,5234,26991,24752,8907,}, 0, 0},
	},
	[24944] = {
			{"ability", "effect_mod", 0.06, {33763,740,5185,8936,}, 0, 0},
			{"ability", "effect_mod_ot", 0.06, {33763,740,774,8936,}, 0, 1},
	},
	[33591] = {
			{"by_attr", "sd_of_stat_pct", 0.25, {4,}, 32, 0},
			{"by_attr", "hp_of_stat_pct", 0.25, {4,}, 32, 1},
	},
	[16859] = {
			{"ability", "base_mod", 0.16, {99,}, 0, 0},
			{"ability", "base_mod_ot", 0.16, {99,}, 0, 0},
			{"ability", "effect_mod", 0.06, {22568,1079,}, 0, 1},
	},
	[17005] = {
			{"by_attr", "stat_mod", 0.12, {4,}, 32, 0},
	},
	[33886] = {
			{"ability", "coef_mod", 0.04, {33763,740,774,8936,}, 0, 0},
	},
	[16910] = {
			{"ability", "crit_mod", 0.39999998, {8921,5176,2912,}, 0, 0},
	},
	[16940] = {
			{"ability", "extra_dur_flat", 0.5, {9005,}, 0, 0},
	},
	[33603] = {
			{"ability", "coef_mod_flat", 0.04, {2912,}, 0, 0},
			{"ability", "coef_mod_flat", 0.02, {5176,}, 0, 1},
	},
	[16860] = {
			{"ability", "base_mod", 0.24, {99,}, 0, 0},
			{"ability", "base_mod_ot", 0.24, {99,}, 0, 0},
			{"ability", "effect_mod", 0.089999996, {22568,1079,}, 0, 1},
	},
	[17073] = {
			{"ability", "cast_mod_flat", -0.5, {5185,}, 0, 0},
			{"raw", "phys_mod", 0.099999994, nil, 1, 1},
	},
	[16862] = {
			{"ability", "base_mod", 0.39999998, {99,}, 0, 0},
			{"ability", "base_mod_ot", 0.39999998, {99,}, 0, 0},
			{"ability", "effect_mod", 0.14999999, {22568,1079,}, 0, 1},
	},
	[17118] = {
			{"ability", "threat", -0.04, {33763,339,740,5185,8921,5176,29166,770,5570,774,16914,2912,18562,8936,}, 0, 0},
	},
	[17050] = {
			{"aura_pts", -1, 0.07, {21850,9885,21849,34906,9884,39233,1126,5232,26990,6756,5234,26991,24752,8907,}, 0, 0},
	},
	[16942] = {
			{"raw", "phys_crit", 0.02, nil, 32, 0},
	},
	[24972] = {
			{"ability", "cost_mod", -0.099999994, {740,5185,}, 0, 0},
	},
	[16847] = {
			{"ability", "cost_mod", -0.089999996, {5185,8921,5176,774,2912,8936,}, 0, 0},
	},
	[16912] = {
			{"ability", "crit_mod", 0.79999995, {8921,5176,2912,}, 0, 0},
	},
	[33597] = {
			{"raw", "mp5_from_int_mod", 0.04, nil, 0, 0},
	},
	[24970] = {
			{"ability", "cost_mod", -0.06, {740,5185,}, 0, 0},
	},
	[17055] = {
			{"aura_pts", -1, 0.35, {21850,9885,21849,34906,9884,39233,1126,5232,26990,6756,5234,26991,24752,8907,}, 0, 0},
	},
	[33879] = {
			{"ability", "coef_mod_flat", 0.099999994, {5185,}, 0, 0},
	},
	[16941] = {
			{"ability", "extra_dur_flat", 1, {9005,}, 0, 0},
	},
	[16948] = {
			{"aura_pts_flat", -1, 0.099999994, {21178,}, 0, 1},
	},
	[17077] = {
			{"ability", "crit", 0.39999998, {8936,}, 0, 0},
	},
	[16900] = {
			{"ability", "effect_mod", 0.08, {8921,5176,2912,}, 0, 0},
			{"ability", "effect_mod_ot", 0.08, {8921,}, 0, 1},
	},
	[35363] = {
			{"ability", "crit", 0.02, {5176,2912,}, 0, 0},
	},
	[33888] = {
			{"ability", "coef_mod", 0.12, {33763,740,774,8936,}, 0, 0},
	},
	[16836] = {
			{"ability", "effect_mod_ot", 0.25, {339,}, 0, 1},
	},
	[16839] = {
			{"ability", "effect_mod_ot", 0.5, {339,}, 0, 1},
	},
	[33592] = {
			{"by_school", "spell_hit", 0.02, {2,3,4,5,6,7,}, 0, 0},
	},
	[16930] = {
			{"raw", "base_res_phys_mod", 0.07, nil, 32, 0},
	},
	[16913] = {
			{"ability", "crit_mod", 1, {8921,5176,2912,}, 0, 0},
	},
	[17072] = {
			{"ability", "cast_mod_flat", -0.4, {5185,}, 0, 0},
			{"raw", "phys_mod", 0.08, nil, 1, 1},
	},
	[17074] = {
			{"ability", "crit", 0.099999994, {8936,}, 0, 0},
	},
	[16909] = {
			{"ability", "crit_mod", 0.19999999, {8921,5176,2912,}, 0, 0},
	},
	[16899] = {
			{"ability", "effect_mod", 0.06, {8921,5176,2912,}, 0, 0},
			{"ability", "effect_mod_ot", 0.06, {8921,}, 0, 1},
	},
	[16911] = {
			{"ability", "crit_mod", 0.59999996, {8921,5176,2912,}, 0, 0},
	},
	[17054] = {
			{"aura_pts", -1, 0.28, {21850,9885,21849,34906,9884,39233,1126,5232,26990,6756,5234,26991,24752,8907,}, 0, 0},
	},
	[24943] = {
			{"ability", "effect_mod", 0.04, {33763,740,5185,8936,}, 0, 0},
			{"ability", "effect_mod_ot", 0.04, {33763,740,774,8936,}, 0, 1},
	},
	[16968] = {
			{"ability", "cost_mod_flat", -18, {5221,}, 0, 0},
			{"ability", "cost_mod_flat", -20, {33745,}, 0, 1},
	},
	[17119] = {
			{"ability", "threat", -0.08, {33763,339,740,5185,8921,5176,29166,770,5570,774,16914,2912,18562,8936,}, 0, 0},
	},
	[33887] = {
			{"ability", "coef_mod", 0.08, {33763,740,774,8936,}, 0, 0},
	},
	[17106] = {
			{"raw", "regen_while_casting", 0.099999994, nil, 0, 0},
	},
	[34153] = {
			{"by_attr", "stat_mod", 0.14999999, {5,}, 32, 0},
	},
	[16817] = {
			{"ability", "cast_mod_flat", -0.4, {5176,2912,}, 0, 0},
	},
	[33604] = {
			{"ability", "coef_mod_flat", 0.08, {2912,}, 0, 0},
			{"ability", "coef_mod_flat", 0.04, {5176,}, 0, 1},
	},
	[16815] = {
			{"ability", "cast_mod_flat", -0.2, {5176,2912,}, 0, 0},
	},
	[24969] = {
			{"ability", "cost_mod", -0.04, {740,5185,}, 0, 0},
	},
	[33881] = {
			{"by_school", "crit", 0.01, {1,2,3,4,5,6,7,}, 32, 1},
	},
	[16846] = {
			{"ability", "cost_mod", -0.06, {5185,8921,5176,774,2912,8936,}, 0, 0},
	},
	[16931] = {
			{"raw", "base_res_phys_mod", 0.099999994, nil, 32, 0},
	},
	[16966] = {
			{"ability", "cost_mod_flat", -9, {5221,}, 0, 0},
			{"ability", "cost_mod_flat", -10, {33745,}, 0, 1},
	},
	[33856] = {
			{"by_attr", "stat_mod", 0.03, {1,2,3,4,5,}, 32, 0},
			{"raw", "attacker_melee_crit", -0.03, nil, 0, 1},
	},
	[24894] = {
			{"by_attr", "stat_mod", 0.19999999, {4,}, 32, 0},
	},
	[33880] = {
			{"ability", "coef_mod_flat", 0.19999999, {5185,}, 0, 0},
	},
	[33590] = {
			{"by_attr", "sd_of_stat_pct", 0.16, {4,}, 32, 0},
			{"by_attr", "hp_of_stat_pct", 0.16, {4,}, 32, 1},
	},
	[33866] = {
			{"by_school", "crit_mod", 0.04, {1,}, 0, 0},
	},
	[16834] = {
			{"ability", "cost_mod", -0.19999999, {783,33943,768,1066,40120,}, 0, 0},
	},
	[16897] = {
			{"ability", "effect_mod", 0.04, {8921,5176,2912,}, 0, 0},
			{"ability", "effect_mod_ot", 0.04, {8921,}, 0, 1},
	},
	[33589] = {
			{"by_attr", "sd_of_stat_pct", 0.08, {4,}, 32, 0},
			{"by_attr", "hp_of_stat_pct", 0.08, {4,}, 32, 1},
	},
	[33889] = {
			{"ability", "coef_mod", 0.16, {33763,740,774,8936,}, 0, 0},
	},
	[16935] = {
			{"ability", "cost_mod_flat", -20, {6807,33878,779,}, 0, 0},
			{"ability", "cost_mod_flat", -2, {1822,1082,33876,}, 0, 1},
	},
	[16949] = {
			{"aura_pts_flat", -1, 0.14999999, {21178,}, 0, 1},
	},
	[33956] = {
			{"raw", "mp5_from_int_mod", 0.099999994, nil, 0, 0},
	},
	[17069] = {
			{"ability", "cast_mod_flat", -0.1, {5185,}, 0, 0},
			{"raw", "phys_mod", 0.02, nil, 1, 1},
	},
	[35364] = {
			{"ability", "crit", 0.04, {5176,2912,}, 0, 0},
	},
	[16936] = {
			{"ability", "cost_mod_flat", -30, {6807,33878,779,}, 0, 0},
			{"ability", "cost_mod_flat", -3, {1822,1082,33876,}, 0, 1},
	},
	[16943] = {
			{"raw", "phys_crit", 0.04, nil, 32, 0},
	},
	[33872] = {
			{"by_attr", "hp_of_stat_pct", 0.5, {2,}, 32, 0},
	},
	[17122] = {
			{"ability", "threat", -0.19999999, {33763,339,740,5185,8921,5176,29166,770,5570,774,16914,2912,18562,8936,}, 0, 0},
	},
	[16833] = {
			{"ability", "cost_mod", -0.099999994, {783,33943,768,1066,40120,}, 0, 0},
	},
	[17104] = {
			{"ability", "effect_mod", 0.02, {33763,740,5185,8936,}, 0, 0},
			{"ability", "effect_mod_ot", 0.02, {33763,740,774,8936,}, 0, 1},
	},
	[34152] = {
			{"by_attr", "stat_mod", 0.099999994, {5,}, 32, 0},
	},
	[24971] = {
			{"ability", "cost_mod", -0.08, {740,5185,}, 0, 0},
	},
	[16896] = {
			{"ability", "effect_mod", 0.02, {8921,5176,2912,}, 0, 0},
			{"ability", "effect_mod_ot", 0.02, {8921,}, 0, 1},
	},
	[24968] = {
			{"ability", "cost_mod", -0.02, {740,5185,}, 0, 0},
	},
	[16944] = {
			{"raw", "phys_crit", 0.06, nil, 32, 0},
	},
	[17113] = {
			{"ability", "effect_mod_ot", 0.14999999, {774,}, 0, 0},
	},
	[33869] = {
			{"by_school", "crit_mod", 0.099999994, {1,}, 0, 0},
	},
	[16840] = {
			{"ability", "effect_mod_ot", 0.75, {339,}, 0, 1},
	},
	[17112] = {
			{"ability", "effect_mod_ot", 0.099999994, {774,}, 0, 0},
	},
};
sc.talent_ranks = {
	[221] = {33917,},
	[111] = {16850,16923,16924,},
	[210] = {16972,16974,16975,},
	[207] = {16979,},
	[316] = {34151,34152,34153,},
	[308] = {16864,},
	[121] = {33831,},
	[313] = {17123,17124,},
	[315] = {17074,17075,17076,17077,17078,},
	[217] = {33851,33852,33957,},
	[102] = {16689,},
	[302] = {17056,17058,17059,17060,17061,},
	[314] = {33879,33880,},
	[317] = {18562,},
	[208] = {16942,16943,16944,},
	[110] = {16909,16910,16911,16912,16913,},
	[201] = {16934,16935,16936,16937,16938,},
	[116] = {33592,33596,},
	[220] = {33859,33866,33867,33868,33869,},
	[112] = {33589,33590,33591,},
	[216] = {33853,33855,33856,},
	[310] = {17111,17112,17113,},
	[206] = {17002,24866,},
	[118] = {24858,},
	[209] = {16966,16968,},
	[312] = {17104,24943,24944,24945,24946,},
	[213] = {16857,},
	[304] = {17063,17065,17066,17067,17068,},
	[108] = {5570,},
	[309] = {24968,24969,24970,24971,24972,},
	[120] = {33603,33604,33605,33606,33607,},
	[303] = {17069,17070,17071,17072,17073,},
	[320] = {33891,},
	[103] = {17245,17247,17248,17249,},
	[113] = {16880,},
	[109] = {16819,16820,},
	[214] = {33872,33873,},
	[101] = {16814,16815,16816,16817,16818,},
	[204] = {16940,16941,},
	[211] = {37116,37117,},
	[215] = {17003,17004,17005,17006,24894,},
	[218] = {17007,},
	[319] = {33886,33887,33888,33889,33890,},
	[119] = {33600,33601,33602,},
	[306] = {17106,17107,17108,},
	[107] = {16836,16839,16840,},
	[219] = {34297,34300,},
	[202] = {16858,16859,16860,16861,16862,},
	[117] = {33597,33599,33956,},
	[301] = {17050,17051,17053,17054,17055,},
	[205] = {16929,16930,16931,},
	[106] = {16821,16822,},
	[203] = {16947,16948,16949,},
	[115] = {16896,16897,16899,16900,16901,},
	[307] = {17118,17119,17120,17121,17122,},
	[104] = {16918,16919,16920,},
	[105] = {35363,35364,},
	[114] = {16845,16846,16847,},
	[311] = {17116,},
	[305] = {16833,16834,16835,},
	[318] = {33881,33882,33883,},
	[212] = {16998,16999,},
};
sc.talent_idx = {
	mangle = 221,
	celestial_focus = 111,
	predatory_strikes = 210,
	feral_charge = 207,
	living_spirit = 316,
	omen_of_clarity = 308,
	force_of_nature = 121,
	improved_tranquility = 313,
	improved_regrowth = 315,
	primal_tenacity = 217,
	natures_grasp = 102,
	furor = 302,
	empowered_touch = 314,
	swiftmend = 317,
	sharpened_claws = 208,
	vengeance = 110,
	ferocity = 201,
	balance_of_power = 116,
	predatory_instincts = 220,
	lunar_guidance = 112,
	survival_of_the_fittest = 216,
	improved_rejuvenation = 310,
	feral_swiftness = 206,
	moonkin_form = 118,
	shredding_attacks = 209,
	gift_of_nature = 312,
	faerie_fire_feral = 213,
	natures_focus = 304,
	insect_swarm = 108,
	tranquil_spirit = 309,
	wrath_of_cenarius = 120,
	naturalist = 303,
	tree_of_life = 320,
	improved_natures_grasp = 103,
	natures_grace = 113,
	natures_reach = 109,
	nurturing_instinct = 214,
	starlight_wrath = 101,
	brutal_impact = 204,
	primal_fury = 211,
	heart_of_the_wild = 215,
	leader_of_the_pack = 218,
	empowered_rejuvenation = 319,
	improved_faerie_fire = 119,
	intensity = 306,
	brambles = 107,
	improved_leader_of_the_pack = 219,
	feral_aggression = 202,
	dreamstate = 117,
	improved_mark_of_the_wild = 301,
	thick_hide = 205,
	improved_moonfire = 106,
	feral_instinct = 203,
	moonfury = 115,
	subtlety = 307,
	control_of_nature = 104,
	focused_starlight = 105,
	moonglow = 114,
	natures_swiftness = 311,
	natural_shapeshifter = 305,
	natural_perfection = 318,
	savage_fury = 212,
};
sc.class_buffs = {
	[9884] = {
			{"by_school", "res_flat", 240, {1,}, 0, 0},
	},
	[16857] = {
			{"by_school", "res_flat", -175, {1,}, 0, 0},
	},
	[5232] = {
			{"by_school", "res_flat", 65, {1,}, 0, 0},
	},
	[9634] = {
			{"applies_aura", "shapeshift_passives", 0, {9635,21178,}, 16, 0},
	},
	[1066] = {
			{"applies_aura", "shapeshift_passives", 0, {5421,}, 16, 0},
	},
	[6793] = {
			{"raw", "phys_dmg_flat", 20, nil, 0, 0},
	},
	[24752] = {
			{"by_school", "res_flat", 285, {1,}, 0, 0},
	},
	[33943] = {
			{"applies_aura", "shapeshift_passives", 0, {33948,34764,}, 16, 0},
	},
	[26990] = {
			{"by_school", "res_flat", 340, {1,}, 0, 0},
	},
	[39233] = {
			{"by_school", "res_flat", 340, {1,}, 0, 0},
			{"by_attr", "stat_flat", 14, {1,2,3,4,5,}, 2, 1},
	},
	[46833] = {
			{"ability", "cast_mod_flat", -1.5000001, {2912,}, 64, 0},
	},
	[17392] = {
			{"by_school", "res_flat", -505, {1,}, 0, 0},
	},
	[8907] = {
			{"by_school", "res_flat", 195, {1,}, 0, 0},
	},
	[24974] = {
			{"raw", "phys_hit", -0.02, nil, 0, 1},
	},
	[45281] = {
			{"raw", "player_vuln_phys", -0.02, nil, 1, 0},
	},
	[770] = {
			{"by_school", "res_flat", -175, {1,}, 0, 0},
	},
	[24858] = {
			{"applies_aura", "shapeshift_passives", 0, {24905,}, 16, 0},
	},
	[17401] = {
			{"raw", "melee_haste", -0.25, nil, 33, 1},
	},
	[40090] = {
			{"raw", "melee_haste", -0.66999996, nil, 33, 1},
	},
	[34906] = {
			{"by_school", "res_flat", 110, {4,}, 0, 0},
	},
	[9845] = {
			{"raw", "phys_dmg_flat", 30, nil, 0, 0},
	},
	[24542] = {
			{"raw", "spell_haste_rating_flat", 70, nil, 2, 0},
			{"ability", "cost_mod", -0.049999997, {740,5185,774,8936,}, 0, 1},
	},
	[24907] = {
			{"by_school", "crit", 0.049999997, {1,2,3,4,5,6,7,}, 32, 0},
			{"raw", "spell_crit_rating_flat", 0, nil, 2, 1},
	},
	[17116] = {
			{"ability", "cast_mod", -1, {339,5185,5176,8936,}, 64, 0},
	},
	[5217] = {
			{"raw", "phys_dmg_flat", 10, nil, 0, 0},
	},
	[19634] = {
			{"raw", "mp5_flat", 83.33333, nil, 0, 0},
	},
	[5570] = {
			{"raw", "phys_hit", -0.02, nil, 0, 1},
	},
	[40120] = {
			{"applies_aura", "shapeshift_passives", 0, {40121,40122,}, 16, 0},
	},
	[37343] = {
			{"by_school", "sp_dmg_flat", 209, {1,2,3,4,5,6,7,}, 2, 0},
	},
	[27012] = {
			{"raw", "melee_haste", -0.25, nil, 33, 1},
	},
	[17391] = {
			{"by_school", "res_flat", -395, {1,}, 0, 0},
	},
	[5234] = {
			{"by_school", "res_flat", 150, {1,}, 0, 0},
	},
	[45282] = {
			{"raw", "player_vuln_phys", -0.03, nil, 1, 0},
	},
	[37316] = {
			{"ability", "cast_mod_flat", -2, {8936,}, 64, 0},
	},
	[24867] = {
			{"raw", "dodge", 0.02, nil, 2, 0},
	},
	[17402] = {
			{"raw", "melee_haste", -0.25, nil, 33, 1},
	},
	[24864] = {
			{"raw", "dodge", 0.04, nil, 2, 0},
	},
	[17390] = {
			{"by_school", "res_flat", -285, {1,}, 0, 0},
	},
	[16870] = {
			{"ability", "cost_mod", -1, {22570,33763,99,339,740,5185,8921,5221,5176,22568,1822,6785,5570,6807,774,1082,16914,9005,2912,33876,33878,18562,779,1079,8936,33745,}, 64, 0},
	},
	[37342] = {
			{"raw", "healing_power_flat", 326, nil, 2, 0},
	},
	[28790] = {
			{"by_school", "res_flat", 700, {1,}, 0, 0},
	},
	[21793] = {
			{"raw", "melee_haste", -3, nil, 33, 2},
	},
	[37878] = {
			{"ability", "cost_mod_flat", -22, {33763,740,5185,774,18562,8936,}, 0, 0},
	},
	[37325] = {
			{"ability", "cost_mod_flat", -450, {8936,}, 64, 0},
	},
	[45283] = {
			{"raw", "player_vuln_phys", -0.04, nil, 1, 0},
	},
	[37341] = {
			{"by_attr", "stat_flat", 64, {1,}, 2, 0},
	},
	[6756] = {
			{"by_school", "res_flat", 105, {1,}, 0, 0},
	},
	[26993] = {
			{"by_school", "res_flat", -610, {1,}, 0, 0},
	},
	[9885] = {
			{"by_school", "res_flat", 285, {1,}, 0, 0},
	},
	[778] = {
			{"by_school", "res_flat", -285, {1,}, 0, 0},
	},
	[27013] = {
			{"raw", "phys_hit", -0.02, nil, 0, 1},
	},
	[21849] = {
			{"by_school", "res_flat", 240, {1,}, 0, 0},
	},
	[23724] = {
			{"ability", "cost_mod_flat", -550, {783,33943,768,1066,40120,}, 0, 0},
	},
	[26991] = {
			{"by_school", "res_flat", 340, {1,}, 0, 0},
	},
	[37721] = {
			{"ability", "cost_mod_flat", -450, {33763,740,5185,774,18562,8936,}, 64, 0},
	},
	[24975] = {
			{"raw", "phys_hit", -0.02, nil, 0, 1},
	},
	[16914] = {
			{"raw", "melee_haste", -0.25, nil, 33, 1},
	},
	[27011] = {
			{"by_school", "res_flat", -610, {1,}, 0, 0},
	},
	[768] = {
			{"applies_aura", "shapeshift_passives", 0, {3025,}, 16, 0},
	},
	[40389] = {
			{"ability", "cost_mod_flat", -115, {33943,40120,}, 0, 0},
	},
	[24932] = {
			{"raw", "phys_crit", 0.049999997, nil, 32, 0},
			{"raw", "melee_crit_rating_flat", 0, nil, 2, 2},
			{"raw", "ranged_crit_rating_flat", 0, nil, 2, 2},
	},
	[1126] = {
			{"by_school", "res_flat", 25, {1,}, 0, 0},
	},
	[22812] = {
			{"raw", "player_vuln_phys", -0.19999999, nil, 1, 1},
	},
	[24976] = {
			{"raw", "phys_hit", -0.02, nil, 0, 1},
	},
	[21850] = {
			{"by_school", "res_flat", 285, {1,}, 0, 0},
	},
	[33891] = {
			{"applies_aura", "shapeshift_passives", 0, {}, 16, 0},
	},
	[32717] = {
			{"raw", "melee_haste", -0.66999996, nil, 33, 1},
	},
	[16886] = {
			{"ability", "cast_mod_flat", -0.5, {339,5185,8921,5176,2912,8936,}, 64, 0},
	},
	[783] = {
			{"applies_aura", "shapeshift_passives", 0, {5419,}, 16, 0},
	},
	[28827] = {
			{"by_school", "res_flat", 700, {1,}, 0, 0},
	},
	[37344] = {
			{"by_school", "sp_dmg_flat", 175, {1,2,3,4,5,6,7,}, 2, 0},
			{"raw", "healing_power_flat", 175, nil, 2, 1},
	},
	[5487] = {
			{"applies_aura", "shapeshift_passives", 0, {1178,21178,}, 16, 0},
	},
	[27555] = {
			{"raw", "res_phys_mod", -0.75, nil, 32, 1},
	},
	[46834] = {
			{"ability", "cast_mod_flat", -0.2, {8936,}, 0, 0},
	},
	[9749] = {
			{"by_school", "res_flat", -395, {1,}, 0, 0},
	},
	[9846] = {
			{"raw", "phys_dmg_flat", 40, nil, 0, 0},
	},
	[9907] = {
			{"by_school", "res_flat", -505, {1,}, 0, 0},
	},
	[24977] = {
			{"raw", "phys_hit", -0.02, nil, 0, 1},
	},
	[27530] = {
			{"raw", "melee_haste", -0.66999996, nil, 33, 1},
	},
};
local class_hostile_buffs = {
	[16857] = {
			{"by_school", "target_res_flat", -175, {1,}, 0, 0},
	},
	[33878] = {
			{"raw", "vuln_bleed", 0.29999998, nil, 1, 1},
			{"ability", "vuln_mod", 0.29999998, {5221,}, 1, 1},
	},
	[33982] = {
			{"raw", "vuln_bleed", 0.29999998, nil, 1, 1},
			{"ability", "vuln_mod", 0.29999998, {5221,}, 1, 1},
	},
	[39233] = {
			{"by_school", "target_res_flat", 340, {1,}, 0, 0},
	},
	[17392] = {
			{"by_school", "target_res_flat", -505, {1,}, 0, 0},
	},
	[45281] = {
			{"by_school", "vuln_mod", -0.02, {1,2,3,4,5,6,7,}, 1, 0},
			{"raw", "vuln_phys", -0.02, nil, 1, 0},
	},
	[770] = {
			{"by_school", "target_res_flat", -175, {1,}, 0, 0},
	},
	[17391] = {
			{"by_school", "target_res_flat", -395, {1,}, 0, 0},
	},
	[45282] = {
			{"by_school", "vuln_mod", -0.03, {1,2,3,4,5,6,7,}, 1, 0},
			{"raw", "vuln_phys", -0.03, nil, 1, 0},
	},
	[33986] = {
			{"raw", "vuln_bleed", 0.29999998, nil, 1, 1},
			{"ability", "vuln_mod", 0.29999998, {5221,}, 1, 1},
	},
	[17390] = {
			{"by_school", "target_res_flat", -285, {1,}, 0, 0},
	},
	[33983] = {
			{"raw", "vuln_bleed", 0.29999998, nil, 1, 1},
			{"ability", "vuln_mod", 0.29999998, {5221,}, 1, 1},
	},
	[45283] = {
			{"by_school", "vuln_mod", -0.04, {1,2,3,4,5,6,7,}, 1, 0},
			{"raw", "vuln_phys", -0.04, nil, 1, 0},
	},
	[33987] = {
			{"raw", "vuln_bleed", 0.29999998, nil, 1, 1},
			{"ability", "vuln_mod", 0.29999998, {5221,}, 1, 1},
	},
	[26993] = {
			{"by_school", "target_res_flat", -610, {1,}, 0, 0},
	},
	[778] = {
			{"by_school", "target_res_flat", -285, {1,}, 0, 0},
	},
	[27011] = {
			{"by_school", "target_res_flat", -610, {1,}, 0, 0},
	},
	[22812] = {
			{"by_school", "vuln_mod", -0.19999999, {1,2,3,4,5,6,7,}, 1, 1},
			{"raw", "vuln_phys", -0.19999999, nil, 1, 1},
	},
	[27546] = {
			{"by_school", "vuln_mod", -0.5, {1,2,3,4,5,6,7,}, 1, 1},
	},
	[9749] = {
			{"by_school", "target_res_flat", -395, {1,}, 0, 0},
	},
	[33876] = {
			{"raw", "vuln_bleed", 0.29999998, nil, 1, 1},
			{"ability", "vuln_mod", 0.29999998, {5221,}, 1, 1},
	},
	[9907] = {
			{"by_school", "target_res_flat", -505, {1,}, 0, 0},
	},
};
for k, v in pairs(class_hostile_buffs) do sc.hostile_buffs[k] = v; end
local class_friendly_buffs = {
	[34123] = {
			{"raw", "healing_power_flat", 1, nil, 0, 0},
	},
};
for k, v in pairs(class_friendly_buffs) do sc.friendly_buffs[k] = v; end
local set_items = {
	[29064] = 624,
	[16827] = 204,
	[16729] = 188,
	[29039] = 633,
	[17620] = 390,
	[21352] = 507,
	[21395] = 506,
	[31379] = 685,
	[28350] = 653,
	[22492] = 521,
	[16732] = 189,
	[22497] = 526,
	[22070] = 518,
	[35411] = 2014,
	[19833] = 477,
	[27708] = 581,
	[16444] = 388,
	[22469] = 527,
	[23273] = 544,
	[16918] = 210,
	[31042] = 676,
	[21860] = 556,
	[16483] = 384,
	[16672] = 187,
	[22510] = 529,
	[21861] = 556,
	[22093] = 516,
	[15056] = 142,
	[16431] = 282,
	[34527] = 675,
	[22426] = 528,
	[12939] = 41,
	[29524] = 618,
	[29608] = 717,
	[16710] = 184,
	[23262] = 540,
	[13218] = 65,
	[22505] = 529,
	[31397] = 686,
	[22086] = 516,
	[22061] = 515,
	[23274] = 544,
	[29038] = 633,
	[14614] = 123,
	[20059] = 471,
	[35357] = 2025,
	[22488] = 521,
	[27870] = 661,
	[25697] = 575,
	[22879] = 522,
	[25687] = 573,
	[16406] = 282,
	[30167] = 634,
	[14637] = 121,
	[29515] = 616,
	[30235] = 643,
	[30979] = 672,
	[29028] = 631,
	[21995] = 511,
	[16677] = 186,
	[29602] = 697,
	[17562] = 346,
	[29040] = 633,
	[17579] = 392,
	[24261] = 571,
	[27883] = 583,
	[21863] = 557,
	[22514] = 525,
	[35393] = 2017,
	[19953] = 477,
	[34448] = 668,
	[16416] = 343,
	[31063] = 675,
	[22843] = 543,
	[12426] = 321,
	[30980] = 673,
	[16530] = 361,
	[23060] = 524,
	[16921] = 211,
	[16823] = 204,
	[34571] = 678,
	[14638] = 121,
	[22860] = 542,
	[34566] = 684,
	[16901] = 214,
	[21868] = 558,
	[29056] = 664,
	[30151] = 665,
	[16494] = 382,
	[29081] = 651,
	[27797] = 659,
	[22874] = 543,
	[29598] = 718,
	[29023] = 655,
	[18205] = 261,
	[20057] = 467,
	[16808] = 203,
	[32945] = 719,
	[30972] = 672,
	[22091] = 516,
	[27705] = 582,
	[22111] = 513,
	[16565] = 396,
	[28225] = 661,
	[22875] = 543,
	[22515] = 525,
	[16391] = 343,
	[20150] = 483,
	[34547] = 673,
	[31039] = 676,
	[17569] = 346,
	[29088] = 638,
	[21855] = 555,
	[14641] = 121,
	[30169] = 635,
	[16800] = 201,
	[23252] = 543,
	[16735] = 189,
	[16485] = 341,
	[29065] = 624,
	[21996] = 511,
	[21412] = 508,
	[17586] = 391,
	[34563] = 674,
	[27909] = 630,
	[16539] = 387,
	[22421] = 523,
	[22073] = 518,
	[16963] = 218,
	[19863] = 465,
	[20047] = 473,
	[31004] = 669,
	[16809] = 203,
	[19689] = 442,
	[23082] = 535,
	[25855] = 579,
	[14620] = 124,
	[19693] = 444,
	[29050] = 663,
	[16692] = 182,
	[29520] = 617,
	[22072] = 518,
	[16396] = 348,
	[20041] = 467,
	[30149] = 622,
	[29061] = 624,
	[29078] = 648,
	[21390] = 505,
	[10331] = 163,
	[21392] = 495,
	[27803] = 653,
	[29084] = 651,
	[22083] = 514,
	[30189] = 636,
	[16708] = 184,
	[29054] = 663,
	[34549] = 669,
	[30139] = 652,
	[22482] = 524,
	[23078] = 533,
	[23291] = 546,
	[19954] = 478,
	[21410] = 508,
	[31378] = 685,
	[23290] = 546,
	[17581] = 392,
	[7952] = 221,
	[19959] = 482,
	[22419] = 523,
	[31027] = 668,
	[23313] = 548,
	[16941] = 215,
	[19682] = 421,
	[15062] = 143,
	[35341] = 2024,
	[23286] = 545,
	[31011] = 682,
	[22501] = 526,
	[30140] = 652,
	[28414] = 620,
	[16919] = 211,
	[16540] = 387,
	[14622] = 124,
	[23514] = 564,
	[34441] = 672,
	[16467] = 395,
	[29095] = 639,
	[16705] = 183,
	[31046] = 677,
	[16397] = 381,
	[27837] = 659,
	[16916] = 210,
	[31053] = 670,
	[31022] = 683,
	[17572] = 345,
	[35376] = 2020,
	[31005] = 669,
	[30207] = 649,
	[16830] = 205,
	[22060] = 515,
	[21407] = 494,
	[35383] = 2019,
	[28968] = 645,
	[16678] = 186,
	[34546] = 672,
	[21373] = 501,
	[19617] = 478,
	[35389] = 2018,
	[21847] = 552,
	[16484] = 384,
	[16519] = 301,
	[16867] = 209,
	[16930] = 212,
	[16514] = 281,
	[16905] = 213,
	[17567] = 346,
	[29077] = 648,
	[21331] = 496,
	[30142] = 652,
	[35391] = 2017,
	[29043] = 633,
	[15046] = 490,
	[16566] = 396,
	[20407] = 492,
	[22498] = 526,
	[16543] = 383,
	[24555] = 568,
	[16956] = 217,
	[22438] = 530,
	[35387] = 2018,
	[23310] = 547,
	[19836] = 478,
	[23259] = 538,
	[22109] = 513,
	[31409] = 687,
	[27536] = 662,
	[14632] = 122,
	[16670] = 187,
	[16401] = 362,
	[22465] = 527,
	[20058] = 468,
	[35331] = 2022,
	[35404] = 2015,
	[16693] = 182,
	[34445] = 678,
	[22867] = 538,
	[21330] = 496,
	[29616] = 698,
	[30141] = 652,
	[16694] = 182,
	[23315] = 545,
	[11728] = 1,
	[34433] = 679,
	[22852] = 539,
	[19577] = 474,
	[16843] = 207,
	[35332] = 2022,
	[16548] = 383,
	[17570] = 345,
	[23506] = 562,
	[31041] = 678,
	[22107] = 513,
	[23285] = 548,
	[30196] = 649,
	[31016] = 683,
	[16666] = 187,
	[17568] = 346,
	[7950] = 221,
	[21351] = 507,
	[28275] = 650,
	[22099] = 519,
	[34447] = 671,
	[27469] = 580,
	[16489] = 341,
	[22417] = 523,
	[27907] = 658,
	[16846] = 206,
	[16855] = 208,
	[30988] = 681,
	[23263] = 542,
	[16554] = 398,
	[28964] = 645,
	[31070] = 674,
	[29604] = 697,
	[29058] = 664,
	[35415] = 2016,
	[16898] = 214,
	[19831] = 477,
	[30997] = 680,
	[31015] = 682,
	[23061] = 525,
	[23092] = 535,
	[16734] = 189,
	[16456] = 394,
	[27879] = 583,
	[34437] = 684,
	[29075] = 626,
	[19958] = 480,
	[29071] = 626,
	[16472] = 402,
	[16810] = 203,
	[23507] = 562,
	[19865] = 461,
	[22483] = 524,
	[16436] = 401,
	[25694] = 576,
	[12428] = 321,
	[22062] = 517,
	[16853] = 208,
	[16726] = 188,
	[27497] = 661,
	[23253] = 539,
	[16541] = 383,
	[16433] = 401,
	[24262] = 559,
	[15055] = 141,
	[22064] = 517,
	[30211] = 646,
	[34556] = 676,
	[20044] = 470,
	[20186] = 486,
	[16949] = 216,
	[29083] = 651,
	[16952] = 217,
	[29022] = 655,
	[22004] = 512,
	[30118] = 657,
	[16725] = 188,
	[7949] = 221,
	[31376] = 685,
	[23085] = 536,
	[19594] = 480,
	[21873] = 554,
	[30123] = 628,
	[17573] = 345,
	[29596] = 718,
	[17583] = 392,
	[29074] = 626,
	[24554] = 568,
	[22017] = 515,
	[29059] = 664,
	[32837] = 699,
	[16727] = 188,
	[30214] = 646,
	[29597] = 718,
	[27702] = 582,
	[23283] = 547,
	[21387] = 505,
	[19692] = 443,
	[35377] = 2020,
	[16687] = 181,
	[20406] = 492,
	[16477] = 384,
	[30114] = 656,
	[22512] = 525,
	[21403] = 510,
	[27528] = 660,
	[22478] = 524,
	[35372] = 2026,
	[29072] = 626,
	[29066] = 625,
	[31007] = 683,
	[21414] = 504,
	[19905] = 462,
	[25673] = 614,
	[22092] = 516,
	[35375] = 2026,
	[35333] = 2023,
	[20195] = 484,
	[30231] = 643,
	[35367] = 2013,
	[30113] = 656,
	[16733] = 189,
	[23309] = 551,
	[34703] = 737,
	[16573] = 386,
	[22436] = 530,
	[25834] = 577,
	[28126] = 584,
	[7953] = 221,
	[30969] = 672,
	[16571] = 396,
	[20055] = 469,
	[23279] = 550,
	[30989] = 680,
	[16414] = 343,
	[25690] = 574,
	[22089] = 516,
	[35346] = 2021,
	[16799] = 201,
	[30996] = 681,
	[34488] = 679,
	[16415] = 343,
	[13389] = 81,
	[23081] = 534,
	[22443] = 530,
	[30144] = 622,
	[16857] = 208,
	[30206] = 649,
	[25856] = 579,
	[22303] = 520,
	[22001] = 511,
	[22431] = 528,
	[29603] = 697,
	[35409] = 2014,
	[34565] = 683,
	[19691] = 443,
	[22108] = 513,
	[21405] = 498,
	[27703] = 582,
	[16392] = 348,
	[16955] = 217,
	[27707] = 581,
	[16907] = 213,
	[18203] = 261,
	[16850] = 206,
	[16934] = 212,
	[29019] = 655,
	[30122] = 657,
	[22887] = 538,
	[25660] = 612,
	[19828] = 476,
	[16805] = 203,
	[20048] = 467,
	[16863] = 209,
	[16796] = 201,
	[16814] = 202,
	[16820] = 204,
	[31023] = 684,
	[13388] = 81,
	[30970] = 673,
	[21360] = 497,
	[21350] = 507,
	[19846] = 482,
	[23292] = 550,
	[22885] = 540,
	[29035] = 632,
	[34528] = 674,
	[23280] = 551,
	[11729] = 1,
	[30171] = 635,
	[17588] = 391,
	[25668] = 613,
	[22305] = 520,
	[21329] = 496,
	[19956] = 476,
	[35395] = 2017,
	[31411] = 687,
	[16722] = 188,
	[29089] = 638,
	[16842] = 207,
	[22011] = 515,
	[16479] = 384,
	[23524] = 565,
	[21348] = 507,
	[35386] = 2018,
	[35342] = 2024,
	[19605] = 481,
	[16462] = 395,
	[22502] = 526,
	[10329] = 163,
	[23494] = 561,
	[29612] = 698,
	[16868] = 209,
	[23261] = 540,
	[29031] = 631,
	[29094] = 639,
	[20053] = 472,
	[30146] = 622,
	[30137] = 627,
	[29594] = 718,
	[16521] = 301,
	[25654] = 611,
	[16821] = 204,
	[16528] = 361,
	[16701] = 183,
	[19825] = 475,
	[29055] = 663,
	[35403] = 2015,
	[16409] = 401,
	[16903] = 214,
	[15048] = 491,
	[28202] = 637,
	[20171] = 488,
	[30161] = 666,
	[31400] = 686,
	[16421] = 381,
	[12424] = 321,
	[24266] = 559,
	[16865] = 209,
	[16724] = 188,
	[21359] = 497,
	[19686] = 441,
	[17577] = 345,
	[20296] = 490,
	[31014] = 684,
	[28966] = 645,
	[19609] = 476,
	[16689] = 181,
	[16702] = 183,
	[22476] = 524,
	[16717] = 185,
	[21866] = 558,
	[17602] = 389,
	[21871] = 553,
	[27474] = 650,
	[21355] = 493,
	[22304] = 520,
	[24263] = 572,
	[16579] = 386,
	[31069] = 675,
	[6473] = 162,
	[16686] = 181,
	[16947] = 216,
	[16929] = 212,
	[16499] = 347,
	[22422] = 523,
	[20046] = 472,
	[19955] = 479,
	[29029] = 631,
	[22868] = 537,
	[19951] = 474,
	[10412] = 162,
	[19866] = 461,
	[27472] = 580,
	[16526] = 361,
	[29097] = 640,
	[19898] = 466,
	[29093] = 639,
	[22857] = 538,
	[17584] = 392,
	[29076] = 648,
	[20043] = 469,
	[29011] = 654,
	[16937] = 215,
	[13392] = 81,
	[21867] = 558,
	[30229] = 641,
	[16558] = 393,
	[17576] = 345,
	[25695] = 575,
	[17612] = 342,
	[22084] = 514,
	[35416] = 2016,
	[16667] = 187,
	[30148] = 622,
	[12425] = 321,
	[23308] = 551,
	[24264] = 571,
	[23084] = 536,
	[20204] = 487,
	[22110] = 513,
	[32946] = 719,
	[25656] = 611,
	[16527] = 361,
	[16550] = 398,
	[27704] = 582,
	[30125] = 628,
	[15045] = 490,
	[16709] = 184,
	[28230] = 662,
	[28331] = 586,
	[28228] = 650,
	[29032] = 631,
	[21418] = 500,
	[27509] = 620,
	[16860] = 208,
	[17618] = 390,
	[10400] = 161,
	[16965] = 218,
	[31054] = 670,
	[22862] = 543,
	[21391] = 505,
	[20176] = 485,
	[22480] = 524,
	[13390] = 81,
	[14621] = 124,
	[28334] = 586,
	[16502] = 382,
	[16942] = 215,
	[16920] = 211,
	[16563] = 393,
	[22500] = 526,
	[29521] = 617,
	[30213] = 646,
	[24546] = 567,
	[16912] = 210,
	[23255] = 541,
	[25657] = 611,
	[30117] = 656,
	[16465] = 395,
	[16723] = 188,
	[25659] = 612,
	[30133] = 629,
	[27977] = 653,
	[16430] = 282,
	[31018] = 682,
	[34560] = 679,
	[30129] = 629,
	[35328] = 2022,
	[16811] = 202,
	[16516] = 281,
	[28137] = 585,
	[24552] = 568,
	[21346] = 503,
	[20212] = 487,
	[16948] = 216,
	[30991] = 679,
	[27709] = 581,
	[16856] = 208,
	[30166] = 634,
	[23491] = 561,
	[15066] = 144,
	[23065] = 527,
	[30145] = 622,
	[16515] = 281,
	[22470] = 527,
	[25692] = 576,
	[35363] = 2027,
	[16831] = 205,
	[16864] = 209,
	[20208] = 487,
	[16501] = 382,
	[34568] = 673,
	[16839] = 207,
	[16457] = 394,
	[22069] = 517,
	[27738] = 647,
	[16902] = 214,
	[35382] = 2019,
	[23087] = 533,
	[22468] = 527,
	[19839] = 479,
	[16393] = 381,
	[22068] = 517,
	[29017] = 654,
	[19684] = 421,
	[21404] = 498,
	[21334] = 499,
	[19834] = 478,
	[22490] = 521,
	[34431] = 680,
	[34545] = 682,
	[22008] = 512,
	[29048] = 621,
	[22494] = 521,
	[22007] = 512,
	[31032] = 678,
	[24547] = 567,
	[16909] = 213,
	[16899] = 214,
	[29610] = 717,
	[31055] = 671,
	[16518] = 301,
	[31058] = 671,
	[22516] = 525,
	[16828] = 205,
	[28128] = 584,
	[25655] = 611,
	[16915] = 210,
	[14612] = 123,
	[16852] = 206,
	[17607] = 389,
	[27880] = 583,
	[17592] = 391,
	[21862] = 556,
	[31375] = 685,
	[16706] = 185,
	[19841] = 480,
	[16716] = 185,
	[22420] = 523,
	[10330] = 163,
	[16817] = 202,
	[34434] = 674,
	[11726] = 1,
	[34561] = 680,
	[23489] = 560,
	[23512] = 563,
	[16815] = 202,
	[17616] = 342,
	[21353] = 493,
	[31407] = 686,
	[23484] = 560,
	[35390] = 2018,
	[29047] = 621,
	[16410] = 401,
	[10403] = 161,
	[24255] = 570,
	[16807] = 203,
	[27796] = 658,
	[15053] = 141,
	[14636] = 121,
	[29012] = 654,
	[30188] = 615,
	[16924] = 211,
	[14616] = 123,
	[23066] = 528,
	[22508] = 529,
	[23294] = 551,
	[22076] = 518,
	[21875] = 554,
	[34436] = 670,
	[28963] = 645,
	[35413] = 2016,
	[16908] = 213,
	[34570] = 669,
	[16966] = 218,
	[35402] = 2015,
	[14623] = 124,
	[16437] = 388,
	[21333] = 496,
	[34554] = 678,
	[21370] = 509,
	[22439] = 530,
	[31066] = 675,
	[35381] = 2019,
	[24545] = 567,
	[16441] = 388,
	[16533] = 387,
	[16698] = 183,
	[27739] = 623,
	[23312] = 548,
	[16480] = 384,
	[17593] = 391,
	[21367] = 509,
	[16578] = 386,
	[34572] = 677,
	[29082] = 651,
	[16946] = 216,
	[20159] = 485,
	[16561] = 393,
	[16897] = 214,
	[10399] = 161,
	[22479] = 524,
	[22873] = 537,
	[28138] = 585,
	[23303] = 549,
	[22504] = 529,
	[16923] = 211,
	[21850] = 555,
	[16945] = 216,
	[28413] = 662,
	[23316] = 549,
	[12427] = 321,
	[23304] = 546,
	[16812] = 202,
	[16510] = 281,
	[13391] = 81,
	[16403] = 362,
	[30992] = 681,
	[16715] = 185,
	[29069] = 625,
	[16806] = 203,
	[16419] = 348,
	[16496] = 382,
	[28139] = 585,
	[21874] = 554,
	[16440] = 388,
	[22427] = 528,
	[25685] = 573,
	[19896] = 463,
	[23314] = 545,
	[16435] = 401,
	[35330] = 2022,
	[22306] = 520,
	[30163] = 666,
	[30222] = 641,
	[16833] = 205,
	[10410] = 162,
	[31020] = 684,
	[34567] = 682,
	[20045] = 471,
	[16714] = 185,
	[30131] = 629,
	[23305] = 546,
	[17578] = 392,
	[16944] = 216,
	[22881] = 541,
	[16525] = 361,
	[21854] = 555,
	[16851] = 206,
	[31047] = 678,
	[21354] = 493,
	[19827] = 475,
	[25831] = 577,
	[22075] = 518,
	[21848] = 552,
	[22442] = 530,
	[19823] = 474,
	[22010] = 515,
	[21998] = 511,
	[29015] = 654,
	[29085] = 651,
	[15063] = 143,
	[28415] = 644,
	[31008] = 684,
	[27936] = 660,
	[19893] = 462,
	[22499] = 526,
	[16840] = 207,
	[35370] = 2013,
	[34443] = 669,
	[29600] = 697,
	[27537] = 644,
	[31406] = 686,
	[22077] = 518,
	[16442] = 388,
	[23089] = 534,
	[30974] = 673,
	[27838] = 647,
	[31012] = 683,
	[23302] = 549,
	[30994] = 681,
	[28224] = 659,
	[23490] = 561,
	[29519] = 617,
	[29033] = 632,
	[17580] = 392,
	[20061] = 473,
	[25696] = 575,
	[22088] = 516,
	[31044] = 676,
	[27535] = 623,
	[21365] = 509,
	[23513] = 564,
	[16954] = 217,
	[19621] = 477,
	[16524] = 301,
	[34432] = 681,
	[28349] = 630,
	[21400] = 502,
	[23311] = 547,
	[34444] = 676,
	[20175] = 488,
	[16536] = 387,
	[23510] = 563,
	[35338] = 2024,
	[21396] = 506,
	[35392] = 2017,
	[16854] = 208,
	[23257] = 522,
	[23515] = 564,
	[23093] = 534,
	[16503] = 382,
	[16938] = 215,
	[22877] = 539,
	[23272] = 544,
	[28130] = 584,
	[31057] = 671,
	[16801] = 201,
	[19685] = 441,
	[16906] = 213,
	[16476] = 402,
	[16699] = 183,
	[34562] = 675,
	[30172] = 635,
	[22440] = 530,
	[16534] = 387,
	[22416] = 523,
	[16845] = 206,
	[23278] = 550,
	[31051] = 670,
	[22074] = 518,
	[19832] = 477,
	[18202] = 261,
	[16713] = 184,
	[29522] = 618,
	[16858] = 208,
	[31029] = 668,
	[16574] = 386,
	[21278] = 142,
	[35359] = 2025,
	[23298] = 548,
	[35361] = 2027,
	[16951] = 217,
	[30217] = 642,
	[17611] = 342,
	[22856] = 522,
	[16449] = 397,
	[35368] = 2013,
	[22302] = 520,
	[16922] = 211,
	[26001] = 578,
	[29516] = 616,
	[30187] = 615,
	[22005] = 512,
	[29526] = 619,
	[16704] = 183,
	[15054] = 141,
	[31338] = 667,
	[23063] = 529,
	[17590] = 391,
	[21345] = 503,
	[19840] = 479,
	[16984] = 489,
	[29067] = 625,
	[31068] = 675,
	[29100] = 640,
	[13183] = 65,
	[23519] = 569,
	[22493] = 521,
	[19822] = 474,
	[32838] = 699,
	[16958] = 217,
	[20158] = 483,
	[11731] = 1,
	[30220] = 642,
	[35406] = 2015,
	[34485] = 680,
	[19835] = 478,
	[31339] = 667,
	[16835] = 205,
	[15067] = 144,
	[25997] = 578,
	[25675] = 614,
	[30219] = 642,
	[22311] = 520,
	[22016] = 515,
	[28403] = 661,
	[27465] = 658,
	[22496] = 526,
	[21870] = 553,
	[23243] = 537,
	[21343] = 503,
	[20051] = 470,
	[23254] = 539,
	[16711] = 184,
	[20154] = 483,
	[29090] = 638,
	[29523] = 618,
	[20163] = 485,
	[35378] = 2020,
	[20042] = 468,
	[21864] = 557,
	[27713] = 660,
	[25676] = 614,
	[24553] = 568,
	[21393] = 495,
	[31050] = 670,
	[22886] = 542,
	[35335] = 2023,
	[29046] = 621,
	[16413] = 343,
	[10413] = 162,
	[16567] = 396,
	[30233] = 643,
	[16446] = 394,
	[16474] = 402,
	[22000] = 511,
	[16718] = 185,
	[30115] = 656,
	[16508] = 347,
	[25674] = 614,
	[16829] = 205,
	[30234] = 643,
	[29517] = 616,
	[34543] = 683,
	[16832] = 213,
	[23258] = 522,
	[22113] = 513,
	[22858] = 537,
	[22090] = 516,
	[10411] = 162,
	[16448] = 397,
	[14640] = 121,
	[14633] = 122,
	[28189] = 737,
	[19695] = 444,
	[35366] = 2013,
	[35336] = 2023,
	[16859] = 208,
	[16551] = 398,
	[16452] = 397,
	[30185] = 636,
	[21853] = 555,
	[29607] = 717,
	[23288] = 549,
	[22865] = 541,
	[22002] = 512,
	[21394] = 495,
	[16505] = 347,
	[16728] = 188,
	[16940] = 215,
	[20050] = 469,
	[23088] = 535,
	[22430] = 528,
	[31059] = 671,
	[31001] = 669,
	[21398] = 502,
	[23293] = 550,
	[17596] = 344,
	[16862] = 209,
	[23318] = 546,
	[16736] = 189,
	[22080] = 514,
	[23307] = 550,
	[21852] = 555,
	[30152] = 665,
	[21999] = 511,
	[29042] = 633,
	[16847] = 206,
	[22424] = 528,
	[22082] = 514,
	[23067] = 530,
	[16950] = 216,
	[29034] = 632,
	[16673] = 187,
	[26000] = 578,
	[16471] = 402,
	[21375] = 501,
	[22464] = 527,
	[16720] = 185,
	[20194] = 486,
	[27471] = 580,
	[28191] = 658,
	[23297] = 547,
	[16425] = 362,
	[16544] = 383,
	[20034] = 482,
	[28348] = 637,
	[16683] = 181,
	[25662] = 612,
	[30223] = 641,
	[20167] = 488,
	[29057] = 664,
	[25671] = 613,
	[30232] = 643,
	[27737] = 637,
	[29060] = 664,
	[16900] = 214,
	[30194] = 636,
	[29036] = 632,
	[22112] = 513,
	[21417] = 500,
	[16964] = 218,
	[31618] = 690,
	[23517] = 569,
	[30982] = 680,
	[16674] = 186,
	[7948] = 221,
	[23091] = 536,
	[28229] = 647,
	[35343] = 2021,
	[15057] = 142,
	[34439] = 682,
	[27801] = 650,
	[30975] = 672,
	[14626] = 122,
	[27531] = 659,
	[20190] = 486,
	[22864] = 522,
	[27710] = 581,
	[31003] = 669,
	[21401] = 510,
	[30976] = 673,
	[27908] = 620,
	[16564] = 393,
	[16568] = 396,
	[21362] = 497,
	[23319] = 546,
	[16911] = 213,
	[28203] = 623,
	[23062] = 526,
	[29525] = 619,
	[19588] = 475,
	[28204] = 620,
	[21865] = 557,
	[23295] = 551,
	[30215] = 646,
	[22066] = 517,
	[22437] = 530,
	[25830] = 577,
	[22423] = 523,
	[27874] = 650,
	[16679] = 186,
	[28129] = 584,
	[22100] = 519,
	[16700] = 183,
	[16957] = 217,
	[31006] = 669,
	[16849] = 206,
	[21332] = 496,
	[21336] = 499,
	[31410] = 687,
	[20295] = 491,
	[35369] = 2013,
	[21368] = 509,
	[22418] = 523,
	[19690] = 443,
	[25693] = 576,
	[21347] = 503,
	[34564] = 670,
	[23521] = 566,
	[31024] = 682,
	[30205] = 649,
	[28278] = 647,
	[16454] = 394,
	[35360] = 2025,
	[29053] = 663,
	[16798] = 201,
	[22513] = 525,
	[28192] = 660,
	[16838] = 207,
	[22101] = 519,
	[24249] = 570,
	[16475] = 402,
	[30230] = 641,
	[22872] = 537,
	[16731] = 189,
	[16691] = 182,
	[22441] = 530,
	[29595] = 718,
	[16580] = 386,
	[35344] = 2021,
	[17610] = 342,
	[21409] = 494,
	[30993] = 680,
	[21413] = 504,
	[16803] = 203,
	[30154] = 665,
	[16931] = 212,
	[18204] = 261,
	[23525] = 565,
	[28332] = 586,
	[16685] = 181,
	[22003] = 512,
	[30153] = 665,
	[29099] = 640,
	[7951] = 221,
	[20033] = 481,
	[16506] = 347,
	[30978] = 673,
	[19842] = 480,
	[27473] = 580,
	[12422] = 321,
	[20203] = 484,
	[17599] = 344,
	[21364] = 497,
	[16451] = 397,
	[10328] = 163,
	[16478] = 384,
	[35371] = 2026,
	[21349] = 507,
	[27706] = 582,
	[16681] = 186,
	[16459] = 397,
	[21399] = 502,
	[30186] = 615,
	[30127] = 628,
	[30119] = 657,
	[34542] = 684,
	[16935] = 215,
	[21389] = 505,
	[31037] = 678,
	[29086] = 638,
	[29601] = 697,
	[16824] = 204,
	[29080] = 648,
	[17598] = 344,
	[21402] = 510,
	[35412] = 2016,
	[24556] = 568,
	[16797] = 201,
	[31377] = 685,
	[16684] = 181,
	[21406] = 498,
	[22869] = 540,
	[21388] = 505,
	[16936] = 215,
	[28232] = 644,
	[16943] = 216,
	[23516] = 564,
	[23277] = 544,
	[17608] = 389,
	[25691] = 574,
	[21846] = 552,
	[35347] = 2021,
	[16428] = 362,
	[31028] = 668,
	[16422] = 381,
	[23518] = 569,
	[16669] = 187,
	[35365] = 2027,
	[30200] = 615,
	[17613] = 342,
	[27776] = 620,
	[22081] = 514,
	[30173] = 635,
	[29049] = 663,
	[30190] = 636,
	[22507] = 529,
	[16668] = 187,
	[22313] = 520,
	[25669] = 613,
	[19838] = 479,
	[16555] = 398,
	[30143] = 652,
	[17624] = 390,
	[27711] = 581,
	[17564] = 346,
	[21338] = 499,
	[30987] = 679,
	[27510] = 630,
	[15058] = 142,
	[16549] = 398,
	[28127] = 584,
	[29617] = 698,
	[10333] = 163,
	[29611] = 717,
	[16688] = 181,
	[34555] = 677,
	[16424] = 381,
	[30192] = 636,
	[22471] = 527,
	[27839] = 623,
	[29045] = 621,
	[30998] = 679,
	[16523] = 301,
	[29613] = 698,
	[35408] = 2014,
	[22880] = 522,
	[16682] = 181,
	[29606] = 717,
	[25857] = 579,
	[31065] = 674,
	[30138] = 627,
	[30221] = 642,
	[28335] = 586,
	[31396] = 686,
	[16453] = 394,
	[35380] = 2020,
	[29599] = 718,
	[23296] = 547,
	[34575] = 668,
	[29073] = 626,
	[21397] = 506,
	[30136] = 627,
	[16487] = 341,
	[27778] = 644,
	[22863] = 539,
	[16491] = 341,
	[19829] = 476,
	[22467] = 527,
	[21374] = 501,
	[35384] = 2019,
	[21859] = 556,
	[30995] = 679,
	[34559] = 681,
	[27470] = 580,
	[16737] = 189,
	[16962] = 218,
	[23251] = 543,
	[16721] = 184,
	[28967] = 645,
	[16420] = 348,
	[23284] = 548,
	[16703] = 183,
	[24544] = 567,
	[16429] = 282,
	[21366] = 509,
	[23488] = 560,
	[35374] = 2026,
	[22301] = 520,
	[17571] = 345,
	[31017] = 684,
	[31064] = 674,
	[22481] = 524,
	[29087] = 638,
	[23487] = 560,
	[35407] = 2014,
	[16560] = 393,
	[25661] = 612,
	[29030] = 631,
	[22519] = 525,
	[34573] = 676,
	[16418] = 348,
	[16535] = 387,
	[27508] = 647,
	[16463] = 395,
	[28401] = 660,
	[16841] = 207,
	[14629] = 122,
	[15049] = 491,
	[22087] = 516,
	[10402] = 161,
	[19848] = 481,
	[19910] = 463,
	[30985] = 679,
	[29609] = 717,
	[22097] = 519,
	[22006] = 512,
	[22489] = 521,
	[12940] = 41,
	[35388] = 2018,
	[16816] = 202,
	[23301] = 545,
	[16707] = 184,
	[29527] = 619,
	[17603] = 389,
	[16959] = 218,
	[27802] = 630,
	[34557] = 671,
	[16822] = 204,
	[30134] = 627,
	[31043] = 677,
	[19845] = 482,
	[19925] = 466,
	[22506] = 529,
	[28333] = 586,
	[23509] = 563,
	[17604] = 389,
	[14631] = 122,
	[16818] = 210,
	[19613] = 479,
	[30212] = 646,
	[23059] = 523,
	[16443] = 388,
	[21997] = 511,
	[16542] = 383,
	[19824] = 474,
	[31619] = 690,
	[27775] = 662,
	[31034] = 676,
	[30116] = 656,
	[25689] = 574,
	[30168] = 634,
	[16466] = 395,
	[27882] = 583,
	[22870] = 542,
	[21337] = 499,
	[16468] = 395,
	[15052] = 489,
	[16712] = 184,
	[22095] = 519,
	[16423] = 381,
	[31052] = 670,
	[19826] = 475,
	[31045] = 678,
	[30170] = 635,
	[16690] = 182,
	[34541] = 670,
	[34435] = 675,
	[16427] = 362,
	[35340] = 2024,
	[16680] = 186,
	[27881] = 583,
	[35339] = 2024,
	[23306] = 550,
	[29070] = 625,
	[17064] = 241,
	[35373] = 2026,
	[30162] = 666,
	[17082] = 241,
	[35385] = 2019,
	[16432] = 282,
	[25998] = 578,
	[16926] = 211,
	[31056] = 671,
	[17594] = 344,
	[21994] = 511,
	[22067] = 517,
	[16861] = 209,
	[22466] = 527,
	[22009] = 512,
	[23493] = 561,
	[29063] = 624,
	[16925] = 211,
	[17601] = 344,
	[16960] = 218,
	[23260] = 538,
	[29091] = 639,
	[29092] = 639,
	[23511] = 563,
	[23287] = 545,
	[35329] = 2022,
	[16819] = 202,
	[30977] = 672,
	[31616] = 690,
	[16910] = 213,
	[25858] = 579,
	[20056] = 470,
	[30160] = 666,
	[22883] = 542,
	[29614] = 698,
	[21335] = 499,
	[16492] = 341,
	[19920] = 465,
	[30150] = 665,
	[22884] = 541,
	[16848] = 206,
	[16695] = 182,
	[16562] = 393,
	[22518] = 525,
	[30201] = 615,
	[20054] = 473,
	[16473] = 402,
	[22855] = 541,
	[15050] = 489,
	[23282] = 547,
	[25686] = 573,
	[16450] = 397,
	[31061] = 674,
	[16569] = 396,
	[29044] = 621,
	[35405] = 2015,
	[27475] = 653,
	[20199] = 484,
	[19694] = 444,
	[19952] = 475,
	[31049] = 677,
	[20408] = 492,
	[29021] = 655,
	[24267] = 572,
	[16804] = 203,
	[11730] = 1,
	[16577] = 386,
	[16927] = 212,
	[16904] = 214,
	[19830] = 476,
	[16504] = 382,
	[16730] = 189,
	[35356] = 2025,
	[16498] = 347,
	[29605] = 697,
	[27875] = 662,
	[31048] = 676,
	[22503] = 526,
	[17625] = 390,
	[30120] = 657,
	[25670] = 613,
	[31021] = 682,
	[17605] = 389,
	[19912] = 464,
	[28193] = 658,
	[23508] = 562,
	[35414] = 2016,
	[29016] = 654,
	[30165] = 634,
	[29020] = 655,
	[35394] = 2017,
	[19873] = 464,
	[17622] = 390,
	[19601] = 482,
	[28285] = 623,
	[22878] = 539,
	[34442] = 673,
	[16697] = 182,
	[16836] = 205,
	[16417] = 348,
	[28231] = 630,
	[22071] = 518,
	[30228] = 641,
	[19688] = 442,
	[14615] = 123,
	[22491] = 521,
	[16434] = 401,
	[22098] = 519,
	[10401] = 161,
	[31060] = 675,
	[16509] = 281,
	[35337] = 2023,
	[31412] = 687,
	[16913] = 210,
	[22876] = 538,
	[23090] = 533,
	[35334] = 2023,
	[22065] = 517,
	[25833] = 577,
	[22015] = 515,
	[35345] = 2021,
	[17623] = 390,
	[23275] = 544,
	[22102] = 519,
	[34438] = 683,
	[16513] = 281,
	[22429] = 528,
	[27948] = 644,
	[31035] = 677,
	[35362] = 2027,
	[31040] = 677,
	[30130] = 629,
	[16405] = 282,
	[16825] = 204,
	[33173] = 566,
	[16719] = 185,
	[23482] = 560,
	[23300] = 545,
	[34569] = 672,
	[16671] = 187,
	[16545] = 383,
	[20049] = 468,
	[35358] = 2025,
	[17617] = 342,
	[10332] = 163,
	[19957] = 481,
	[30990] = 680,
	[16844] = 207,
	[35410] = 2014,
	[16676] = 186,
	[21372] = 501,
	[27873] = 637,
	[21849] = 555,
	[30135] = 627,
	[16455] = 394,
	[22511] = 529,
	[16837] = 207,
	[12429] = 321,
	[23276] = 544,
	[29037] = 632,
	[28140] = 585,
	[21411] = 508,
	[16933] = 212,
	[20060] = 472,
	[17566] = 346,
	[16675] = 186,
	[31413] = 687,
	[22106] = 513,
	[16552] = 398,
	[19687] = 441,
	[30126] = 628,
	[16369] = 343,
	[15051] = 489,
	[29068] = 625,
	[22495] = 521,
	[21869] = 553,
	[34446] = 677,
	[16914] = 210,
	[30132] = 629,
	[22078] = 514,
	[34558] = 668,
	[30121] = 657,
	[20052] = 471,
	[30216] = 642,
	[34487] = 681,
	[16426] = 362,
	[29615] = 698,
	[23520] = 566,
	[16826] = 204,
	[22882] = 540,
	[30164] = 634,
	[31614] = 690,
	[23522] = 566,
	[31067] = 674,
	[29098] = 640,
	[28205] = 653,
	[31030] = 668,
	[21416] = 500,
	[21361] = 497,
	[21376] = 501,
	[25832] = 577,
	[30210] = 649,
	[16834] = 205,
	[30983] = 681,
	[21408] = 494,
	[24549] = 567,
	[22063] = 517,
	[19843] = 480,
	[30124] = 628,
	[21851] = 555,
	[22085] = 514,
	[17591] = 391,
	[16932] = 212,
	[28264] = 659,
	[27468] = 637,
	[31019] = 683,
	[22013] = 515,
	[28136] = 585,
	[17600] = 344,
	[34574] = 671,
	[25999] = 578,
	[35364] = 2027,
	[16953] = 217,
	[21357] = 493,
	[16917] = 210,
	[21415] = 504,
	[16795] = 201,
	[14624] = 124,
	[21344] = 503,
	[23289] = 549,
	[22477] = 524,
	[16961] = 218,
	[16813] = 202,
	[22425] = 528,
	[19849] = 481,
	[27771] = 661,
	[16507] = 347,
	[16866] = 209,
	[23256] = 541,
	[16928] = 212,
	[22079] = 514,
	[16802] = 201,
	[22096] = 519,
	[22517] = 525,
	[23281] = 551,
	[16696] = 182,
	[29062] = 624,
	[31613] = 690,
	[21356] = 493,
	[16522] = 301,
	[23244] = 537,
	[23317] = 549,
	[22509] = 529,
	[30159] = 666,
	[14611] = 123,
	[35379] = 2020,
	[23064] = 521,
	[31026] = 668,
	[29079] = 648,
	[23264] = 542,
	[22859] = 540,
	[23299] = 548,
	[16531] = 361,
	[22428] = 528,
	[23523] = 565,
	[16939] = 215,
	[16490] = 341,
	[25854] = 579,
	[19683] = 421,
	[29096] = 640,
};
for k, v in pairs(set_items) do sc.set_items[k] = v; end
local set_bonuses = {
	[461] = {{2,41874},},
	[626] = {{2,37186},{4,37187},},
	[679] = {{2,38421},{4,38422},},
	[2026] = {{2,46436},},
	[183] = {{4,41736},},
	[2025] = {{2,46435},},
	[504] = {{3,26131},},
	[582] = {{2,40044},},
	[665] = {{4,26171},},
	[677] = {{2,38414},{4,38415},},
	[725] = {{2,43478},},
	[744] = {{2,46437},{4,46834},},
	[680] = {{4,38424},},
	[750] = {{2,46528},},
	[344] = {{2,41805},{4,23044},{6,41804},},
	[601] = {{2,40046},},
	[398] = {{2,41903},{6,41904},},
	[512] = {{6,41722},},
	[524] = {{6,28811},{8,28814},},
	[588] = {{2,40050},},
	[701] = {{2,41464},},
	[583] = {{2,32145},},
	[209] = {{8,23561},},
	[580] = {{2,40042},},
	[480] = {{2,41718},{5,24483},},
	[216] = {{3,21899},{5,23570},},
	[705] = {{2,41463},{4,33333},},
	[387] = {{2,41901},{6,41902},},
	[163] = {{3,41641},{6,41683},},
	[201] = {{3,41657},{5,41656},{8,23545},},
	[361] = {{2,41701},{6,41700},},
	[540] = {{2,41697},{4,23044},{6,41696},},
	[564] = {{3,41769},},
	[468] = {{2,41640},{3,41041},},
	[207] = {{8,23573},},
	[630] = {{2,37207},{4,37209},},
	[688] = {{2,40045},{4,46834},},
	[394] = {{2,41767},{6,41768},},
	[720] = {{2,43478},},
	[741] = {{2,46415},},
	[390] = {{2,41899},{3,23044},{6,41900},},
	[553] = {{3,39372},},
	[557] = {{3,41841},},
	[622] = {{2,37169},},
	[603] = {{2,40047},{4,22804},},
	[397] = {{2,41764},{6,41763},},
	[507] = {{3,26169},{5,26171},},
	[649] = {{2,37441},},
	[647] = {{2,37423},},
	[660] = {{2,37609},},
	[625] = {{2,37184},{4,37185},},
	[681] = {{2,38426},{4,38425},},
	[730] = {{2,43479},},
	[124] = {{2,41726},{5,41723},},
	[740] = {{2,46414},{4,33333},},
	[584] = {{2,32145},},
	[608] = {{2,40050},},
	[2018] = {{2,46454},{4,22804},},
	[2022] = {{2,46412},{4,23047},},
	[444] = {{3,41847},},
	[747] = {{2,46455},},
	[573] = {{3,41746},},
	[574] = {{3,41846},},
	[301] = {{2,41693},{4,22804},{6,41692},},
	[613] = {{2,41878},{4,41877},},
	[619] = {{3,41832},},
	[123] = {{2,41671},{3,41672},{5,41674},},
	[487] = {{2,41853},{3,39486},},
	[544] = {{2,41807},{6,41806},},
	[552] = {{3,32196},},
	[486] = {{2,41852},{3,39545},},
	[620] = {{4,37166},},
	[683] = {{2,38434},{4,38435},},
	[675] = {{2,38410},{4,38411},},
	[731] = {{2,43478},},
	[735] = {{2,43480},{4,23047},},
	[2019] = {{2,46456},},
	[628] = {{2,37190},{4,37191},},
	[2024] = {{2,46414},{4,33333},},
	[321] = {{4,41647},{6,41778},},
	[348] = {{2,41817},{6,41816},},
	[587] = {{2,40052},},
	[2014] = {{2,46528},},
	[41] = {{2,41719},},
	[550] = {{2,41800},{6,41801},},
	[624] = {{2,37182},},
	[443] = {{3,41675},},
	[655] = {{2,37518},},
	[527] = {{2,28818},{4,29171},{6,28823},{8,28821},},
	[742] = {{2,46435},},
	[714] = {{2,41464},},
	[181] = {{4,30777},},
	[212] = {{3,32449},{8,23555},},
	[737] = {{2,9336},},
	[505] = {{3,26130},{5,41648},},
	[678] = {{4,38420},},
	[382] = {{2,41711},{6,41710},},
	[721] = {{2,43479},},
	[627] = {{2,37188},},
	[513] = {{6,41752},{6,41646},},
	[693] = {{2,40045},{4,46851},},
	[554] = {{3,32102},},
	[599] = {{2,40047},},
	[702] = {{2,41474},{4,23047},},
	[644] = {{4,37376},},
	[500] = {{3,26118},},
	[571] = {{2,32200},},
	[656] = {{2,37522},},
	[706] = {{2,41464},},
	[691] = {{2,40045},{4,33333},},
	[686] = {{2,40043},},
	[2023] = {{2,46413},{4,33333},},
	[465] = {{2,41830},},
	[208] = {{5,41785},{5,41784},},
	[490] = {{2,41775},{3,21894},},
	[2016] = {{2,46534},},
	[592] = {{2,40051},{4,23047},},
	[213] = {{5,23558},},
	[545] = {{2,41790},{6,41791},},
	[597] = {{2,40047},{4,33333},},
	[469] = {{2,41865},{3,39418},},
	[342] = {{2,41703},{4,23044},{6,41702},},
	[347] = {{2,41717},{6,41716},},
	[514] = {{6,41884},},
	[542] = {{2,41687},{6,41652},},
	[577] = {{2,32145},},
	[391] = {{2,41906},{3,23047},{6,41905},},
	[204] = {{8,23582},},
	[503] = {{3,26127},},
	[581] = {{2,40042},{4,33333},},
	[749] = {{2,46456},},
	[122] = {{2,41642},{3,41827},{5,41825},},
	[541] = {{2,41654},{4,23047},{6,41691},},
	[421] = {{3,41676},},
	[481] = {{2,41734},{3,24486},},
	[594] = {{2,40050},},
	[281] = {{2,41690},{6,41653},},
	[575] = {{3,41749},},
	[598] = {{2,40051},{4,33333},},
	[609] = {{2,40047},{4,33877},},
	[343] = {{2,41811},{6,41810},},
	[556] = {{3,39881},},
	[215] = {{3,23559},},
	[621] = {{2,37167},},
	[671] = {{2,38396},{4,38397},},
	[690] = {{2,40043},{4,46851},},
	[685] = {{2,40043},{4,46834},},
	[694] = {{2,40049},{4,46851},},
	[710] = {{2,41462},{4,44302},},
	[2015] = {{2,46530},{4,46851},},
	[218] = {{3,23563},},
	[689] = {{2,40049},{4,46834},},
	[525] = {{2,28807},{4,28809},{6,28808},},
	[586] = {{2,32145},},
	[188] = {{4,41818},},
	[509] = {{3,26173},},
	[2027] = {{2,46437},{4,46834},},
	[144] = {{2,41780},},
	[548] = {{2,41796},{6,41797},},
	[389] = {{2,41759},{3,23044},{6,41760},},
	[612] = {{2,41833},{4,41834},},
	[670] = {{4,38393},},
	[585] = {{2,40042},},
	[653] = {{2,37512},{4,37513},},
	[716] = {{2,41462},{4,44293},},
	[519] = {{6,41860},},
	[633] = {{2,37223},{4,37224},},
	[684] = {{4,38436},},
	[709] = {{2,41463},},
	[206] = {{3,23560},{8,23566},},
	[282] = {{2,41793},{6,41792},},
	[704] = {{2,41462},{4,23047},},
	[607] = {{2,40046},},
	[590] = {{2,40046},},
	[555] = {{2,39543},{4,41829},},
	[589] = {{2,40048},},
	[591] = {{2,40047},{4,23047},},
	[682] = {{2,38429},},
	[345] = {{2,41715},{4,23047},{6,41714},},
	[187] = {{4,41856},},
	[81] = {{4,41872},{5,41869},},
	[476] = {{2,41659},{3,24461},},
	[610] = {{2,40051},{4,33877},},
	[658] = {{2,37607},},
	[667] = {{2,41875},},
	[551] = {{2,41808},{6,41809},},
	[184] = {{4,41835},},
	[396] = {{2,41898},{6,41897},},
	[162] = {{2,41740},{3,41739},{4,41644},{5,41741},},
	[569] = {{3,41743},},
	[497] = {{5,26113},},
	[618] = {{3,41591},},
	[695] = {{2,40045},},
	[708] = {{2,41463},{4,46851},},
	[711] = {{2,41464},},
	[712] = {{2,41462},},
	[733] = {{2,43480},},
	[475] = {{2,41770},{3,24457},{5,24460},},
	[518] = {{6,41729},},
	[529] = {{2,28831},{4,28829},{6,28746},{8,28830},},
	[748] = {{2,46456},},
	[143] = {{2,39516},},
	[217] = {{5,41782},{8,23591},},
	[614] = {{2,41890},{4,41889},},
	[751] = {{2,46530},},
	[752] = {{2,46534},},
	[617] = {{3,41828},},
	[485] = {{2,41851},{3,41850},},
	[467] = {{2,41868},{3,39548},},
	[511] = {{6,41649},},
	[604] = {{2,40051},{4,22804},},
	[477] = {{2,41831},{5,24467},},
	[517] = {{6,32451},},
	[383] = {{2,41887},{6,41888},},
	[464] = {{2,39880},},
	[596] = {{2,40050},},
	[692] = {{2,40049},{4,33333},},
	[707] = {{2,41462},{4,33333},},
	[616] = {{3,39982},},
	[659] = {{2,37608},},
	[499] = {{3,26116},{5,26117},},
	[528] = {{2,28775},{6,28789},{8,28787},},
	[384] = {{2,41756},{6,41755},},
	[558] = {{3,41658},},
	[567] = {{2,32145},},
	[161] = {{4,41730},{5,41733},},
	[734] = {{2,43481},{4,23047},},
	[515] = {{6,41667},},
	[568] = {{2,40042},{4,23047},},
	[474] = {{2,41885},{5,24431},},
	[543] = {{2,41698},{6,41699},},
	[142] = {{4,41845},},
	[182] = {{4,41879},},
	[698] = {{2,41886},{6,30778},},
	[472] = {{2,41639},{3,39420},},
	[629] = {{2,37194},},
	[562] = {{3,41655},},
	[637] = {{2,37286},{4,37287},},
	[726] = {{2,43479},},
	[738] = {{2,46412},{4,23047},},
	[483] = {{2,41848},{3,39884},},
	[388] = {{2,41761},{6,41762},},
	[214] = {{3,21894},{5,21872},{8,21871},},
	[595] = {{2,40046},},
	[652] = {{2,37381},{4,37505},},
	[743] = {{2,46436},},
	[640] = {{4,37299},},
	[488] = {{2,41855},{3,41854},},
	[521] = {{4,28743},{8,28719},},
	[526] = {{4,28764},},
	[646] = {{2,37381},{4,37384},},
	[579] = {{2,40042},{4,44302},},
	[563] = {{3,41742},},
	[593] = {{2,40046},},
	[462] = {{2,39702},{2,41894},},
	[362] = {{2,41803},{6,41802},},
	[600] = {{2,40051},},
	[696] = {{2,40049},},
	[641] = {{2,37315},{4,37333},},
	[2021] = {{2,46415},},
	[496] = {{3,26109},{5,26110},},
	[492] = {{3,24746},},
	[718] = {{2,41713},{4,22804},{6,41712},},
	[634] = {{2,37225},{4,37227},},
	[729] = {{2,43480},{4,33333},},
	[442] = {{2,41670},{2,41650},},
	[565] = {{3,41783},},
	[566] = {{2,41678},},
	[578] = {{2,32145},},
	[605] = {{2,40046},},
	[657] = {{4,37535},},
	[508] = {{3,26172},},
	[1] = {{3,41864},{4,41861},{5,41862},},
	[189] = {{4,30770},},
	[473] = {{2,41866},{3,39422},},
	[539] = {{2,41707},{6,41706},},
	[602] = {{2,40050},},
	[341] = {{2,41709},{6,41708},},
	[202] = {{3,21973},{5,21092},{8,23550},},
	[560] = {{2,42155},{4,41744},},
	[728] = {{2,43478},{4,33333},},
	[2017] = {{2,46455},},
	[745] = {{2,46438},},
	[713] = {{2,41464},},
	[522] = {{2,41695},{6,41694},},
	[724] = {{2,43480},{4,44302},},
	[651] = {{2,37484},{4,37485},},
	[346] = {{2,41815},{4,23047},{6,41814},},
	[642] = {{2,37313},{4,37314},},
	[401] = {{2,41787},{2,41636},{6,41786},},
	[495] = {{3,26111},},
	[561] = {{2,41561},{4,41562},},
	[650] = {{4,37483},},
	[606] = {{2,40050},},
	[576] = {{3,41771},},
	[727] = {{2,43480},},
	[732] = {{2,43479},},
	[536] = {{3,29068},},
	[395] = {{2,41757},{6,41758},},
	[489] = {{2,39510},{3,39509},},
	[502] = {{3,26123},},
	[535] = {{3,29068},},
	[668] = {{2,38388},{4,38389},},
	[478] = {{2,41821},{5,24471},},
	[537] = {{2,41689},{6,41688},},
	[631] = {{2,37210},},
	[546] = {{2,41789},{6,41788},},
	[381] = {{2,41813},{6,41812},},
	[673] = {{2,38408},{4,38407},},
	[470] = {{2,41638},{3,39447},},
	[700] = {{2,41462},},
	[736] = {{2,43479},},
	[221] = {{6,41772},},
	[632] = {{2,37212},},
	[664] = {{2,37570},{4,37571},},
	[211] = {{3,21894},{8,22010},},
	[674] = {{2,38413},{4,38412},},
	[530] = {{2,28755},{8,28751},},
	[570] = {{2,41876},},
	[676] = {{2,38447},{4,38416},},
	[523] = {{2,28844},{4,28843},{6,28842},},
	[205] = {{3,23557},{5,39507},},
	[402] = {{2,41754},{6,41753},},
	[549] = {{2,41799},{4,23044},{6,41798},},
	[645] = {{2,37377},{2,39437},{4,37380},},
	[687] = {{2,40043},{4,33333},},
	[697] = {{2,41705},{6,41704},},
	[392] = {{2,41765},{3,23047},{6,41766},},
	[661] = {{2,37610},},
	[746] = {{2,46454},{4,22804},},
	[479] = {{2,41776},{3,24479},{5,24480},},
	[466] = {{2,41823},},
	[386] = {{2,41896},{3,22804},{6,41895},},
	[538] = {{2,41713},{4,22804},{6,41712},},
	[2013] = {{2,40046},},
	[722] = {{2,43480},{4,44293},},
	[516] = {{6,41844},},
	[723] = {{2,43479},},
	[471] = {{2,41867},{3,39433},},
	[186] = {{4,30772},},
	[121] = {{2,41681},{3,41645},{5,41679},},
	[491] = {{3,41651},},
	[639] = {{2,37295},},
	[482] = {{2,41777},{3,24489},{5,24491},},
	[185] = {{4,41892},{4,41891},},
	[493] = {{3,41774},},
	[393] = {{2,41908},{6,41907},},
	[638] = {{2,37288},},
	[484] = {{2,41849},{3,41643},},
	[498] = {{3,26114},},
	[533] = {{3,29068},},
	[669] = {{2,38390},{4,38392},},
	[715] = {{2,41463},},
	[717] = {{2,41896},{3,22804},{6,41895},},
	[534] = {{3,29068},},
	[615] = {{2,40053},{4,23047},},
	[739] = {{2,46413},{4,33333},},
	[501] = {{5,26122},},
	[2020] = {{2,46456},},
	[623] = {{2,37180},},
	[663] = {{2,37564},},
	[210] = {{3,21879},},
	[703] = {{2,41464},},
	[547] = {{2,41795},{4,23047},{6,41794},},
	[611] = {{2,41748},{4,41747},},
	[636] = {{4,37241},},
	[672] = {{2,38398},{4,38399},},
	[643] = {{2,37324},},
};
for k, v in pairs(set_bonuses) do sc.set_bonuses[k] = v; end
local items = {
	[33508] = {43736,},
	[31025] = {37760,},
	[27989] = {33565,},
	[28355] = {42367,},
	[30051] = {37737,},
	[186053] = {352574,},
	[33946] = {43842,},
	[23198] = {28855,},
	[23004] = {28847,},
	[186052] = {352504,},
	[23197] = {28854,},
	[33943] = {43844,},
	[33671] = {44835,},
	[32387] = {39926,},
	[34998] = {44835,},
	[31334] = {38314,},
	[27886] = {34246,},
	[33945] = {43840,},
	[8345] = {17768,},
	[186054] = {352508,},
	[35020] = {46090,},
	[28568] = {38321,},
	[32486] = {40442,},
	[27990] = {33565,},
	[33076] = {34128,},
	[33841] = {43725,},
	[33947] = {43843,},
	[22399] = {27846,},
	[32257] = {41037,},
	[19613] = {24433,},
	[33942] = {43841,},
	[35021] = {46100,},
	[27744] = {34253,},
	[35019] = {46088,},
	[22398] = {27853,},
	[22397] = {27851,},
	[25940] = {34323,},
	[33944] = {43845,},
};
for k, v in pairs(items) do if sc.items[k] then for _, vv in ipairs(v) do sc.items[k][#sc.items[k] + 1] = vv; end else sc.items[k] = v; end end
local item_effects = {
	[28854] = {
			{"ability", "sp_flat", 33, {8921,}, 0, 0},
	},
	[24433] = {
			{"raw", "spell_crit_rating_flat", 28, nil, 2, 0},
	},
	[38321] = {
			{"ability", "effect_mod_flat", 136, {5185,}, 0, 0},
	},
	[34253] = {
			{"ability", "base_mod_flat", 8, {33745,}, 0, 0},
			{"ability", "base_mod_ot_flat", 8, {33745,}, 0, 0},
	},
	[39926] = {
			{"aura_pts_flat", 2, 20, {24932,}, 0, 1},
			{"aura_pts_flat", 1, 20, {24907,}, 0, 2},
	},
	[28855] = {
			{"ability", "base_mod_flat", 50, {6807,}, 0, 0},
			{"ability", "base_mod_flat", 10, {779,}, 0, 1},
	},
	[43736] = {
			{"ability", "cost_mod_flat", -36, {774,}, 0, 0},
	},
	[37737] = {
			{"ability", "cost_mod_flat", -65, {8936,}, 0, 0},
	},
	[38314] = {
			{"ability", "cost_mod_flat", -200, {783,33943,768,1066,40120,}, 0, 0},
	},
	[37760] = {
			{"ability", "effect_mod_flat", 25, {5176,}, 0, 0},
	},
	[33565] = {
			{"ability", "base_mod_flat", 30, {1822,1082,}, 0, 0},
	},
	[46100] = {
			{"ability", "base_mod_flat", 131, {33763,}, 0, 0},
	},
	[27846] = {
			{"ability", "effect_mod_flat", 100, {5185,}, 0, 0},
	},
	[27851] = {
			{"ability", "base_mod_flat", 20, {1822,1082,}, 0, 0},
	},
	[27853] = {
			{"ability", "sp_flat", 50, {774,}, 0, 0},
	},
	[352508] = {
			{"ability", "effect_mod_ot_flat", 15, {774,}, 0, 0},
	},
	[352504] = {
			{"ability", "sp_flat", 10, {8921,}, 0, 0},
	},
	[34128] = {
			{"ability", "base_mod_flat", 105, {33763,}, 0, 0},
	},
	[43725] = {
			{"ability", "base_mod_flat", 116, {33763,}, 0, 0},
	},
	[352574] = {
			{"ability", "base_mod_flat", 5, {1822,1082,}, 0, 0},
	},
	[42367] = {
			{"ability", "base_mod_flat", 87, {33763,}, 0, 0},
	},
};
for k, v in pairs(item_effects) do sc.item_effects[k] = v; end
local set_effects = {
	[21872] = {
			{"ability", "cast_mod_flat", -0.2, {8936,}, 0, 0},
	},
	[38416] = {
			{"ability", "effect_mod", 0.14999999, {22568,779,1079,}, 0, 0},
			{"ability", "effect_mod_ot", 0.14999999, {22568,1079,}, 0, 1},
	},
	[37314] = {
			{"ability", "base_mod_flat", 150, {33763,}, 0, 0},
	},
	[24480] = {
			{"ability", "crit", 0.03, {2912,}, 0, 0},
	},
	[21871] = {
			{"ability", "extra_dur_flat", 3.0000002, {774,}, 0, 0},
	},
	[33877] = {
			{"ability", "cost_mod", -0.099999994, {8921,}, 0, 0},
	},
	[37333] = {
			{"ability", "base_mod_flat", 3, {33745,}, 0, 1},
			{"ability", "base_mod_ot_flat", 3, {33745,}, 0, 1},
	},
	[28743] = {
			{"ability", "cost_mod", -0.03, {740,5185,774,8936,}, 0, 0},
	},
	[37287] = {
			{"ability", "cost_mod", -0.25, {783,33943,768,1066,40120,}, 0, 0},
	},
	[38415] = {
			{"ability", "crit", 0.049999997, {2912,}, 0, 0},
	},
	[38420] = {
			{"ability", "effect_mod", 0.049999997, {5185,}, 0, 0},
	},
	[24479] = {
			{"ability", "extra_dur_flat", 5, {770,}, 0, 0},
	},
	[38447] = {
			{"ability", "cost_mod_flat", -5, {33876,}, 0, 0},
			{"ability", "threat", 0.14999999, {33878,}, 0, 1},
	},
	[38414] = {
			{"ability", "extra_dur_flat", 3.0000002, {8921,}, 0, 0},
	},
	[37299] = {
			{"by_attr", "stat_flat", 30, {1,}, 2, 0},
	},
	[46834] = {
			{"ability", "cast_mod_flat", -0.2, {8936,}, 0, 0},
	},
	[37313] = {
			{"ability", "extra_dur_flat", 6.0000005, {8936,}, 0, 0},
	},
};
for k, v in pairs(set_effects) do sc.set_effects[k] = v; end
local enchant_effects = {
};
for k, v in pairs(enchant_effects) do sc.enchant_effects[k] = v; end
local enchants = {
};
for k, v in pairs(enchants) do sc.enchants[k] = v; end
local passives = {
};
for k, v in pairs(passives) do sc.passives[k] = v; end
sc.shapeshift_passives = {
	[1178] = {
			{"raw", "base_res_phys_mod", 1.8, nil, 32, 0},
			{"by_attr", "stat_mod", 0.25, {3,}, 32, 1},
			{"raw", "ap_flat", 30, nil, 258, 2},
	},
	[24905] = {
			{"raw", "base_res_phys_mod", 4, nil, 32, 0},
			{"raw", "ap_flat", 60, nil, 2, 1},
	},
	[21178] = {
			{"by_school", "threat", 0.29999998, {1,2,3,4,5,6,7,}, 0, 0},
	},
	[9635] = {
			{"raw", "base_res_phys_mod", 4, nil, 32, 0},
			{"by_attr", "stat_mod", 0.25, {3,}, 32, 1},
			{"raw", "ap_flat", 120, nil, 258, 2},
	},
	[3025] = {
			{"raw", "ap_flat", 40, nil, 258, 0},
			{"by_school", "threat", -0.29, {1,2,3,4,5,6,7,}, 0, 1},
	},
};
